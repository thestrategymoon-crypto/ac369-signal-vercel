...
export default async function handler(req,res){
  ...
  try{
    ...
    var out={
      ...
      rsi:{'1H':t1h?+t1h.rsi.toFixed(1):50,'4H':t4h?+t4h.rsi.toFixed(1):50,'1D':t1d?+t1d.rsi.toFixed(1):50},
      ict:{
        structure:{bos:bosD||null,choch:chochD||null,label:ictStructure,summary:ictSummary},
        ...
        wyckoff:wyckoff,inOTE:inOTE,oteLabel:oteLabel,
        highProbZones:bullOBs.filter(o=>o.inZone).length>0?['Bullish OB Zone active']:inOTE?['ICT OTE Long Zone']:[]
      },
      obs:{bullish:bullOBs,bearish:bearOBs},orderBlocks:{bullish:bullOBs,bearish:bearOBs},
      fvg:{bullish:bullFVGs,bearish:bearFVGs},
      elliottWave:ew,
      fibonacci:fib,
      pivotPoints:{'4H':pivots},
      macd:{'4H':t4h?{bull:t4h.macdBull,hist:t4h.macdHist,line:t4h.macdLine,cross:t4h.macdCross}:{bull:false,hist:0,line:0,cross:null},bull:t4h?t4h.macdBull:false,hist:t4h?t4h.macdHist:0,line:t4h?t4h.macdLine:0,cross:t4h?t4h.macdCross:null},
      ...
      stochRSI:{'4H':t4h?{k:t4h.stochK,d:t4h.stochD,zone:t4h.stochZone,crossUp:t4h.stochCrossUp,crossDown:t4h.stochCrossDown}:{k:50,d:50,zone:'NEUTRAL',crossUp:false,crossDown:false}},
      adx:{'4H':t4h?{value:t4h.adx,trend:t4h.adxTrend}:{value:25,trend:'WEAK'}},
      atr:{'4H':t4h?{pct:t4h.atrPct,val:t4h.atr}:{pct:0,val:0},pct:t4h?t4h.atrPct:0,val:t4h?t4h.atr:0},
      volume:{'4H':t4h?{ratio:t4h.volRatio,trend:t4h.volTrend}:{ratio:1,trend:'NORMAL'},ratio:t4h?t4h.volRatio:1,trend:t4h?t4h.volTrend:'NORMAL'},
      ...
      chartPatterns:chartPatterns.filter(function(p){return p.winRate>=65;}).sort(function(a,b){return b.winRate-a.winRate;}),
