...
export default async function handler(req,res){
  ...
  try{
    ...
    const [R0,R1,R2,R3,R4,R5,R6,R7,R8,R9,R10,R11,R12,R13,R14,R15,R16,R17]=await Promise.allSettled([
      ...
      get('https://api.mexc.com/api/v3/klines?symbol=BNBUSDT&interval=4h&limit=52',2500),  // R7: BNB 4H Bybit
      get('https://api.mexc.com/api/v3/klines?symbol=XRPUSDT&interval=4h&limit=52',2500),  // R8: XRP 4H Bybit
      get('https://api.mexc.com/api/v3/klines?symbol=LINKUSDT&interval=4h&limit=52',2500), // R9: LINK 4H Bybit
      get('https://api.mexc.com/api/v3/klines?symbol=AVAXUSDT&interval=4h&limit=52',2500), // R10: AVAX 4H Bybit
      ...
      get('https://api.mexc.com/api/v3/klines?symbol=BTCUSDT&interval=1h&limit=20',2000),  // R13: BTC 1H Bybit
      get('https://api.mexc.com/api/v3/klines?symbol=ETHUSDT&interval=1h&limit=20',2000),  // R14: ETH 1H Bybit
      get('https://api.mexc.com/api/v3/klines?symbol=SOLUSDT&interval=1h&limit=20',2000),  // R15: SOL 1H Bybit
      get('https://api.mexc.com/api/v3/klines?symbol=BNBUSDT&interval=1h&limit=20',2000),  // R16: BNB 1H Bybit
      get('https://api.mexc.com/api/v3/klines?symbol=XRPUSDT&interval=1h&limit=20',2000),  // R17: XRP 1H Bybit
    ]);
    ...
    if(phase2Coins.length>0){
      try{
        // Run klines AND funding history CONCURRENTLY (not sequential)
        // Funding history = NEW: check persistence across 6 periods, not just snapshot
        const [p2Results,fundResults]=await Promise.all([
          Promise.allSettled(phase2Coins.map(c=>get('https://api.mexc.com/api/v3/klines?symbol='+c.sym+'USDT&interval=4h&limit=52',2000))),
          Promise.allSettled(phase2Coins.map(c=>get('https://api.bybit.com/v5/market/funding/history?category=linear&symbol='+c.sym+'USDT&limit=6',2000)))
        ]);
        // Parse funding persistence: how many of last 6 periods had negative FR
        const fundPersist={};
        fundResults.forEach((res,i)=>{
          try{
            const sym=phase2Coins[i].sym;
            const list=res&&res.value&&res.value.result&&res.value.result.list;
            if(!list||!list.length)return;
            const negCount=list.filter(x=>+(x.fundingRate||0)<0).length;
            const avgFr=list.reduce((s,x)=>s+(+x.fundingRate||0),0)/list.length;
            fundPersist[sym]={negCount,total:list.length,avgFr:+avgFr.toFixed(6)};
          }catch(e2){}
        });
        p2Results.forEach((res,i)=>{
          try{
            ...
            // ========================================
            // WHALE FOOTPRINT SCORE - Wyckoff-style accumulation detection
            // Uses OBV trend + BB Squeeze + Absorption candles + Vol acceleration
            // All computed from existing 52-candle 4H kline data (no extra API calls)
            // ========================================
            let wfScore=0,wfReasons=[],obvTrend='flat',bbSqueezePct=50,absorptionCount=0,volAccelRatio=1;
            try{
              // 1. OBV (On-Balance Volume) trend - classic Wyckoff accumulation signal
              // If price flat but OBV rising = buying pressure absorbed without moving price
              let obv=0;const obvSeries=[0];
              for(let i=1;i<K.length;i++){
                if(K[i].c>K[i-1].c)obv+=K[i].v;
                else if(K[i].c<K[i-1].c)obv-=K[i].v;
                obvSeries.push(obv);
              }
              ...
              if(obvLen>=20){
                ...
                const obvPriorAvg=obvPrior.reduce((a,b)=>a+b,0)/obvPrior.length;
                const obvDelta=obvRecentAvg-obvPriorAvg;
                const obvRange=Math.max(...obvSeries)-Math.min(...obvSeries)||1;
                const obvDeltaPct=obvDelta/obvRange*100;
                // Price change over same period for comparison
                const priceChgPct=cls.length>=20?((cls[cls.length-1]-cls[cls.length-20])/cls[cls.length-20]*100):0;
                if(obvDeltaPct>8&&Math.abs(priceChgPct)<3){
                  obvTrend='strong_up';wfScore+=40;wfReasons.push('OBV naik kuat tapi harga flat (akumulasi diam-diam)');
                }else if(obvDeltaPct>3&&Math.abs(priceChgPct)<5){
                  obvTrend='up';wfScore+=25;wfReasons.push('OBV trending up, harga belum bergerak');
                }else if(obvDeltaPct>0&&priceChgPct<2){
                  obvTrend='mild_up';wfScore+=12;
                }else if(obvDeltaPct<-5){
                  obvTrend='down';
                }
              }

              // 2. Bollinger Band Squeeze - volatility compression = energy stored
              if(cls.length>=20){
                const bbPeriod=20,bbSlice=cls.slice(-bbPeriod);
                const bbMean=bbSlice.reduce((a,b)=>a+b,0)/bbPeriod;
                const bbVar=bbSlice.reduce((a,b)=>a+(b-bbMean)*(b-bbMean),0)/bbPeriod;
                const bbStd=Math.sqrt(bbVar);
                const bbWidth=bbMean>0?(4*bbStd)/bbMean*100:10;// width as % of price
                // Compare to BB width history (rolling, using all 52 candles)
                const bbWidthHist=[];
                for(let i=bbPeriod;i<cls.length;i++){
                  const seg=cls.slice(i-bbPeriod,i);
                  const m2=seg.reduce((a,b)=>a+b,0)/bbPeriod;
                  const v2=seg.reduce((a,b)=>a+(b-m2)*(b-m2),0)/bbPeriod;
                  const w2=m2>0?(4*Math.sqrt(v2))/m2*100:10;
                  bbWidthHist.push(w2);
                }
                if(bbWidthHist.length>=10){
                  const sorted=bbWidthHist.slice().sort((a,b)=>a-b);
                  const rank=sorted.findIndex(w=>w>=bbWidth);
                  bbSqueezePct=Math.round((rank>=0?rank:sorted.length)/sorted.length*100);
                  if(bbSqueezePct<=15){wfScore+=25;wfReasons.push('BB Squeeze ekstrem ('+bbSqueezePct+'th percentile) - volatilitas terendah');}
                  else if(bbSqueezePct<=30){wfScore+=15;wfReasons.push('BB Squeeze ('+bbSqueezePct+'th percentile)');}
                  else if(bbSqueezePct<=45){wfScore+=6;}
                }
              }

              // 3. Absorption Candles - high volume + small range = supply absorbed
              const last10=K.slice(-10);
              const avgVol10=last10.reduce((s,k)=>s+k.v,0)/last10.length;
              const avgRange10=last10.reduce((s,k)=>s+Math.abs(k.h-k.l),0)/last10.length;
              absorptionCount=last10.filter(k=>{
                const range=Math.abs(k.h-k.l);
                return k.v>avgVol10*1.4&&range<avgRange10*0.7;
              }).length;
              if(absorptionCount>=4){wfScore+=20;wfReasons.push(absorptionCount+' candle absorption (volume tinggi, harga tertahan)');}
              else if(absorptionCount>=2){wfScore+=12;wfReasons.push(absorptionCount+' candle absorption terdeteksi');}
              else if(absorptionCount>=1){wfScore+=5;}

              // 4. Volume Acceleration - recent 3 candles vs prior 10
              const vol3=K.slice(-3).reduce((s,k)=>s+k.v,0)/3;
              const vol10prior=K.slice(-13,-3).reduce((s,k)=>s+k.v,0)/10;
              volAccelRatio=vol10prior>0?+(vol3/vol10prior).toFixed(2):1;
              if(volAccelRatio>=1.5){wfScore+=15;wfReasons.push('Volume akselerasi '+volAccelRatio+'x (mulai aktif)');}
              else if(volAccelRatio>=1.2){wfScore+=8;}

              wfScore=Math.min(100,wfScore);
            }catch(e3){}

            // Multi-day range for Persistent Accumulation Score
            const cls18=cls.slice(-18);// ~3 days of 4H candles
            const hi3d=Math.max(...cls18),lo3d=Math.min(...cls18);
            const range3d=lp>0?(hi3d-lo3d)/lp*100:10;
            const hi9d=Math.max(...cls),lo9d=Math.min(...cls);
            const range9d=lp>0?(hi9d-lo9d)/lp*100:10;
            const fp2=fundPersist[sym]||null;
            km[sym]={rsi:+rsiV.toFixed(2),slopeDir:sDir,slopeTxt:sTxt,macd:{bull:macdLine>sigLine,xUp:macdLine>sigLine&&macdLine<0,xDown:macdLine<sigLine&&macdLine>0,val:+macdLine.toFixed(6)},e9,e21,e200,atr:+atrV.toFixed(8),price:lp,aboveE200:lp>e200,isCoiling:rangeW5<8,vB:vBull>vBear*1.2,volTrend:volTr,div,divStr,rsi1h:null,rsi1d:null,rangeW5:+rangeW5.toFixed(2),src:'p2',wfScore,wfReasons,obvTrend,bbSqueezePct,absorptionCount,volAccelRatio,range3d:+range3d.toFixed(2),range9d:+range9d.toFixed(2),fundNegCount:fp2?fp2.negCount:null,fundTotal:fp2?fp2.total:null,fundAvgFr:fp2?fp2.avgFr:null};
            realRSI++;
          }catch(e2){}
        });
      }catch(e){}
    ...
    for(const[sym,by]of Object.entries(cm)){try{
      ...
      coins.push({
        ...
        wfScore:km2?km2.wfScore||0:0,wfReasons:km2?km2.wfReasons||[]:[],
        obvTrend:km2?km2.obvTrend||'flat':'flat',bbSqueezePct:km2?km2.bbSqueezePct:null,
        absorptionCount:km2?km2.absorptionCount||0:0,volAccelRatio:km2?km2.volAccelRatio||1:1
      });
    }catch(e){}}
    coins.sort((a,b)=>((b.conv&&b.conv.score)||0)-((a.conv&&a.conv.score)||0));

    // LONG filter: quality over quantity
    // Bear market = only real data or anomaly decoupling allowed
    const longs=coins.filter(x=>{
      if(x.direction!=='LONG')return false;
      const cv2=(x.conv&&x.conv.score)||0;if(cv2<60)return false;
      if(bearEstimate){
        return x.rsiReal||(x.decouplingScore||0)>=65||
               x.signal==='WHALE LOADING'||x.signal==='CONFIRMED BREAKOUT'||
               x.signal==='DECOUPLING STRONG'||x.signal==='DECOUPLING';
      }
      return true;
    }).sort((a,b)=>{
      // Sort: decoupling first, then by conv score
      const aD=(a.decouplingScore||0)>=65?1:0;
      const bD=(b.decouplingScore||0)>=65?1:0;
      if(aD!==bD)return bD-aD;
      return((b.conv&&b.conv.score)||0)-((a.conv&&a.conv.score)||0);
    }).slice(0,30);
    ...
    // ALTCOIN SM LOADING RADAR v2
    // FOCUS: coins in LOADING phase (not yet moved)
    // Key insight: high volume + flat price = SM absorbing supply quietly
    const TOP7S=new Set(['BTC','ETH','SOL','BNB','XRP','LINK','AVAX','BTCUSDC','ETHUSDC','BTCUSDC']);
    const smRadar=(()=>{try{
      const candidates=[];
      const bybitSet=new Set();

      // LAYER 1: Bybit futures - LOADING PHASE detection
      // Key: price FLAT + OI rising + FR negative = SM loading
      Object.entries(cm).forEach(([s,by])=>{
        ...
        if(oi<5e6||vol<300000||p<=0)return;
        const range24=p>0&&h>l?(h-l)/p*100:10;
        const volOIr=oi>0?vol/oi:0;
        // PRE-PUMP FILTER: price must NOT have moved yet
        if(Math.abs(c24)>4)return;// exclude already moved
        if(range24>7)return;// exclude wide range = already active
        if(range24<1.2)return;// exclude stablecoins/pegged (too stable)
        const rsiV=by.rsi||50;
        if(rsiV>68||rsiV<28)return;// exclude overbought or extreme oversold
        // Exclude stablecoin-like symbols
        if(s==='EUR'||s==='USD1'||s==='USDC'||s==='USDT'||s==='DAI'||s==='BUSD')return;
        let smS=0,smR=[];
        // CORE: FR negative = retail wrong side = SM contrarian
        if(fp<-0.0003){smS+=35;smR.push('FR '+(fp*100).toFixed(3)+'% (shorts bayar ke SM)');}
        else if(fp<-0.0001){smS+=25;smR.push('FR '+(fp*100).toFixed(3)+'% negatif');}
        else if(fp<0){smS+=15;smR.push('FR sedikit negatif');}
        else if(fp===0){smS+=5;}
        // CORE: OI rising while price flat = new money entering quietly
        const prevO=OI_CACHE.prev[s]||oi;
        const oiD=prevO>0?(oi-prevO)/prevO*100:0;
        if(oiD>3&&Math.abs(c24)<2){smS+=30;smR.push('OI +'+(oiD).toFixed(1)+'% + harga flat = whale masuk');}
        else if(oiD>1&&Math.abs(c24)<3){smS+=20;smR.push('OI +'+(oiD).toFixed(1)+'% akumulasi');}
        else if(oiD>0){smS+=10;}
        // SUPPORTING: tight range = coiling
        if(range24<2){smS+=20;smR.push('Range '+range24.toFixed(1)+'% sangat ketat (coiling)');}
        else if(range24<4){smS+=12;smR.push('Range '+range24.toFixed(1)+'% konsolidasi');}
        else{smS+=5;}
        // SUPPORTING: Vol/OI = fresh activity
        if(volOIr>0.25){smS+=15;smR.push('Vol/OI '+volOIr.toFixed(2)+'x aktif');}
        else if(volOIr>0.1){smS+=8;}
        // RSI sweet spot bonus
        if(rsiV>=40&&rsiV<=55){smS+=10;smR.push('RSI '+rsiV.toFixed(0)+' sweet spot (energi tersimpan)');}
        else if(rsiV>=35&&rsiV<40){smS+=6;}
        if(smS>=40){
          bybitSet.add(s);
          const slPct=Math.max(range24*1.5,2.5);
          const tpPct=range24*3;
          candidates.push({
            sym:s,price:p,c24:+c24.toFixed(2),vol:vol,oi:oi,fr:+(fp*100).toFixed(4),
            oiDelta:+oiD.toFixed(2),range24:+range24.toFixed(2),volOIr:+volOIr.toFixed(3),
            smScore:Math.min(100,smS),reasons:smR,layer:'bybit',rsi:rsiV,
            sl:+(p*(1-slPct/100)).toFixed(p>1?2:8),
            tp1:+(p*(1+Math.max(range24,2)/100)).toFixed(p>1?2:8),
            tp2:+(p*(1+Math.max(tpPct,4)/100)).toFixed(p>1?2:8),
            slPct:+slPct.toFixed(2),tp2Pct:+tpPct.toFixed(2),
            sector:by.sec||'ALT',confirmed:false,
            label:oiD>2&&Math.abs(c24)<2?'LOADING PHASE':range24<3?'PRE-BREAKOUT':'WATCH'
          });
        }
      });

      // LAYER 2: MEXC spot - unusual volume + flat price
      Object.entries(cm).forEach(([s,mx])=>{
        ...
        if(vol<500000||p<=0)return;
        const range24=p>0&&h>l?(h-l)/p*100:10;
        // PRE-PUMP FILTER: must not have moved yet
        if(Math.abs(c24)>4)return;// already moved = exclude
        if(range24>7)return;
        if(range24<1.2)return;// exclude stablecoins
        // Exclude stable-like symbols
        if(/USD|EUR|DAI|BUSD|USDC|USDT/.test(s))return;
        const rsiV=mx.rsi||50;
        if(rsiV>68||rsiV<28)return;
        const volMC=mx.mc>0?vol/mx.mc*100:0;
        let mxS=0,mxR=[];
        // Unusual volume for coin size = key signal
        if(vol>10e6&&volMC>20){mxS+=40;mxR.push('Vol $'+(vol/1e6).toFixed(0)+'M ('+volMC.toFixed(0)+'% MCap) unusual');}
        else if(vol>5e6&&volMC>10){mxS+=28;mxR.push('Vol $'+(vol/1e6).toFixed(0)+'M ('+volMC.toFixed(0)+'% MCap)');}
        else if(vol>2e6&&volMC>5){mxS+=18;mxR.push('Vol $'+(vol/1e6).toFixed(0)+'M elevated');}
        else if(vol>1e6){mxS+=10;}
        // Price flat + volume = accumulation
        if(range24<2&&vol>1e6){mxS+=35;mxR.push('Harga flat '+range24.toFixed(1)+'% + vol aktif = akumulasi');}
        else if(range24<4){mxS+=20;mxR.push('Konsolidasi ketat');}
        // RSI sweet spot
        if(rsiV>=40&&rsiV<=55){mxS+=15;mxR.push('RSI '+rsiV.toFixed(0)+' sweet spot');}
        else if(rsiV>=35){mxS+=8;}
        if(mxS>=45){
          const ex=candidates.find(c2=>c2.sym===s);
          if(ex){ex.smScore=Math.min(100,ex.smScore+12);ex.confirmed=true;ex.reasons=[...ex.reasons,...mxR.slice(0,2)];}
          else{
            const slPct2=Math.max(range24*1.5,2.5);
            candidates.push({
              sym:s,price:p,c24:+c24.toFixed(2),vol:vol,oi:0,fr:null,oiDelta:0,
              range24:+range24.toFixed(2),volOIr:0,smScore:Math.min(100,mxS),
              reasons:mxR,layer:'mexc',rsi:rsiV,
              sl:+(p*(1-slPct2/100)).toFixed(p>1?2:8),
              tp1:+(p*(1+range24/100)).toFixed(p>1?2:8),
              tp2:+(p*(1+range24*3/100)).toFixed(p>1?2:8),
              slPct:+slPct2.toFixed(2),tp2Pct:+(range24*3).toFixed(2),
              sector:mx.sec||'ALT',volMCpct:+volMC.toFixed(1),confirmed:false,
              label:range24<2?'PRE-BREAKOUT':range24<4?'LOADING PHASE':'WATCH'
            });
          }
        }
      });

      return candidates.filter(c=>c.smScore>=45)
        .sort((a,b)=>{
          // CONFIRMED first, then by score
          if(a.confirmed&&!b.confirmed)return -1;
          if(!a.confirmed&&b.confirmed)return 1;
          // PRE-BREAKOUT > LOADING PHASE > WATCH
          const rank={PRE:3,'LOADING':2,WATCH:1};
          const ra=rank[a.label&&a.label.split(' ')[0]]||1;
          const rb=rank[b.label&&b.label.split(' ')[0]]||1;
    ...
    // PERSISTENT ACCUMULATION SCORE - 3 INDEPENDENT confirmations across TIME
    // Not a snapshot - requires patterns sustained over 2-9 days
    // Honest limitation: cannot detect wash trading (no trade-count data available)
    const persistentAccumulation=(()=>{try{
      return coins.filter(c=>c.src==='by'&&(c.oi||0)>30e6).map(c=>{
        const km3=km[c.sym]||null;
        let score=0,reasons=[];
        // 1. FUNDING PERSISTENCE (35pt) - sustained across multiple periods, not snapshot
        if(km3&&km3.fundTotal){
          const ratio=km3.fundNegCount/km3.fundTotal;
          if(ratio>=0.67){score+=35;reasons.push('FR negatif '+km3.fundNegCount+'/'+km3.fundTotal+' periode terakhir (persisten)');}
          else if(ratio>=0.5){score+=22;reasons.push('FR negatif '+km3.fundNegCount+'/'+km3.fundTotal+' periode');}
          else if(ratio>=0.33){score+=10;}
        }
        // 2. OI/VOLUME DIVERGENCE (30pt) - OI growing faster than volume churn
        const oiD=c.oiDelta||0;
        const volOIr=c.oi>0?c.vol/c.oi:0;
        if(oiD>2&&volOIr<0.2){score+=30;reasons.push('OI +'+oiD.toFixed(1)+'% tapi Vol/OI rendah ('+volOIr.toFixed(2)+'x) = limit order pasif');}
        else if(oiD>1&&volOIr<0.3){score+=18;reasons.push('OI naik, churn rendah');}
        else if(oiD>0){score+=8;}
        // 3. MULTI-DAY COMPRESSION (35pt) - BOTH 3-day AND 9-day must be tight
        if(km3&&km3.range3d!=null&&km3.range9d!=null){
          const tight3=km3.range3d<5,tight9=km3.range9d<12;
          if(tight3&&tight9){score+=35;reasons.push('Flat 3 hari ('+km3.range3d+'%) DAN 9 hari ('+km3.range9d+'%) - jarang kebetulan');}
          else if(tight3||tight9){score+=18;reasons.push((tight3?'Flat 3 hari':'Flat 9 hari')+' terdeteksi');}
        }
        return Object.assign({},c,{pasScore:Math.min(100,score),pasReasons:reasons,range3d:km3?km3.range3d:null,range9d:km3?km3.range9d:null,fundNegCount:km3?km3.fundNegCount:null,fundTotal:km3?km3.fundTotal:null});
      }).filter(c=>c.pasScore>=35)
        .sort((a,b)=>b.pasScore-a.pasScore)
        .slice(0,15)
        .map(c=>{
          const atrP=c.atrPct||Math.max(c.range3d||3,2.5);
          const slPct=+(Math.max(atrP*2.2,3.5)).toFixed(2);
          const tpPct=+(atrP*5).toFixed(2);
          return{
            sym:c.sym,sector:c.sector,price:c.price,c24:c.c24,
            pasScore:c.pasScore,pasReasons:c.pasReasons,
            range3d:c.range3d,range9d:c.range9d,
            fundNegCount:c.fundNegCount,fundTotal:c.fundTotal,
            oiDelta:c.oiDelta,vol:c.vol,oi:c.oi,rsi:c.rsi,rsiReal:c.rsiReal,
            label:c.pasScore>=70?'CONFIRMED ACCUM':c.pasScore>=50?'PROBABLE':'WATCH',
            sl:+(c.price*(1-slPct/100)).toFixed(c.price>1?2:8),
            tp1:+(c.price*(1+atrP*2.5/100)).toFixed(c.price>1?2:8),
            tp2:+(c.price*(1+tpPct/100)).toFixed(c.price>1?2:8),
            slPct,tp2Pct:tpPct
          };
        });
    }catch(e){return[]}})();

    // WHALE FOOTPRINT LEADERS - OBV + BB Squeeze + Absorption Wyckoff-style detection
    const whaleFootprintLeaders=(()=>{try{
      return coins
        .filter(c=>(c.wfScore||0)>=40&&c.price>0)
        .sort((a,b)=>(b.wfScore||0)-(a.wfScore||0))
        .slice(0,15)
        .map(c=>{
          const atrP=c.atrPct||Math.max(Math.abs(c.c24||0)*0.6,2);
          const slPct=+(Math.max(atrP*2,3)).toFixed(2);
          const tpPct=+(atrP*5).toFixed(2);
          return{
            sym:c.sym,sector:c.sector,price:c.price,c24:c.c24,
            wfScore:c.wfScore,wfReasons:c.wfReasons||[],
            obvTrend:c.obvTrend,bbSqueezePct:c.bbSqueezePct,
            absorptionCount:c.absorptionCount,volAccelRatio:c.volAccelRatio,
            rsi:c.rsi,rsiReal:c.rsiReal,vol:c.vol,fr:c.fr||null,
            oiDelta:c.oiDelta,
            label:c.wfScore>=70?'STRONG ACCUM':c.wfScore>=55?'ACCUMULATING':'WATCH',
            sl:+(c.price*(1-slPct/100)).toFixed(c.price>1?2:8),
            tp1:+(c.price*(1+atrP*2.5/100)).toFixed(c.price>1?2:8),
            tp2:+(c.price*(1+tpPct/100)).toFixed(c.price>1?2:8),
            slPct,tp2Pct:tpPct
    ...
    const dosBase=50;
    const dosFg=fg<20?20:fg<30?15:fg<40?10:fg>75?-10:0;
    const dosMc=mcType==='MASS OVERSOLD'?20:mcType==='BULLISH'?10:mcType==='BEARISH'?-5:0;
    const dosSess=cso.q==='PRIME'?10:cso.q==='GOOD'?5:cso.q==='POOR'?-10:0;
    const dosRsi=btcK&&btcK.rsi&&btcK.rsi<30?15:btcK&&btcK.rsi&&btcK.rsi<40?8:0;
    const dosScore=Math.min(100,Math.max(0,dosBase+dosFg+dosMc+dosSess+dosRsi));
    const dosLabel=dosScore>=80?'PRIME OPPORTUNITY':dosScore>=65?'GOOD':dosScore>=50?'NORMAL':dosScore>=35?'CAUTION':'POOR';
    const dailyOpportunityScore={score:dosScore,label:dosLabel,action:dosScore>=65?'Setup berkualitas tersedia. Sizing normal.':dosScore>=50?'Selektif. Hanya setup 3+ konfluens.':'Hindari entry baru. Cash is king.'};

    // Market Regime
    const btcRsiNow2=btcK&&btcK.rsi?btcK.rsi:50;
    const isCap2=btcRsiNow2<15&&fg<25;
    const isAccum2=btcRsiNow2<25||(fg<35&&btcRsiNow2<40);
    let regimeKey,regimeColor2,regimeDesc2,sizingG;
    if(isCap2){regimeKey='CAPITULATION';regimeColor2='cyan';regimeDesc2='RSI '+btcRsiNow2.toFixed(0)+' + F&G '+fg+' = CAPITULATION historis. DCA spot.';sizingG='Spot DCA bertahap. NO leverage.';}
    else if(isAccum2){regimeKey='ACCUMULATE';regimeColor2='cyan';regimeDesc2='Extreme oversold = zona akumulasi terbaik.';sizingG='DCA 25-50%. Spot only.';}
    else if(fg>=35&&btcRsiNow2>40&&obCount/totC<0.15&&(cso.q==='PRIME'||cso.q==='GOOD')){regimeKey='TRADE';regimeColor2='green';regimeDesc2='Market seimbang + session bagus = kondisi ideal trading.';sizingG='Full sizing. Patuhi SL.';}
    else if(fg>65||btcRsiNow2>70){regimeKey='CAUTION';regimeColor2='amber';regimeDesc2='Overbought atau euphoria.';sizingG='50% sizing. Ketat SL.';}
    else{regimeKey='AVOID';regimeColor2='red';regimeDesc2='Kondisi tidak ideal untuk trading aktif.';sizingG='Jangan entry baru.';}
    const marketRegime={regime:regimeKey,regimeColor:regimeColor2,regimeDesc:regimeDesc2,sizingGuidance:sizingG};

    // Today's Best Trade
    const todaysBestTrade=(()=>{try{
      const pool=longs.filter(x=>x.levels&&x.levels.tp2>0);
      if(!pool.length)return null;
      const top=pool[0];
      const reasoning=[];
      if(top.rsiReal&&top.rsi<30)reasoning.push('RSI '+top.rsi.toFixed(0)+' REAL oversold (klines data)');
      if(top.rsi1d&&top.rsi1d<35)reasoning.push('RSI 1D: '+top.rsi1d+' bearish zone');
      if(top.mtfAligned==='BULL')reasoning.push('MTF confluence confirmed');
      if(top.divergence==='BULLISH')reasoning.push('Bullish divergence aktif');
      if((top.decouplingScore||0)>=70)reasoning.push('Decoupling +'+top.rs+'% vs BTC');
      const cv2=(top.conv&&top.conv.score)||0;
      return{
        sym:top.sym,signal:top.signal,price:top.price,
        sl:top.levels.sl,tp1:top.levels.tp1,tp2:top.levels.tp2,
        slPct:top.levels.slPct,tp1Pct:top.levels.tp1Pct,tp2Pct:top.levels.tp2Pct,
        rr:top.rr,probability:top.probability||76,
        convStars:top.convStars,
        mtfConfirmed:top.mtfAligned==='BULL',
        reasoning:reasoning.slice(0,5),
        divergence:top.divergence||null,
        retailLong:top.retailLong||null,retailShort:top.retailShort||null,
        retailBias:top.retailBias||null,
        futuresRisk:top.futuresRisk||null,
        kellySizing:{suggestedSizePct:top.kellySizing&&top.kellySizing.suggestedSizePct?top.kellySizing.suggestedSizePct:10},
        regimeWarning:isAccum2?'BEAR MARKET: Sizing 25-50%. Konfirmasi candle dulu.':null
      };
    }catch(e){return null}})();

    const out={
      ok:true,version:'v16.1',ts:Date.now(),elapsed:Date.now()-t0,
