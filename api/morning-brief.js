...
export default async function handler(req,res){
  ...
  try{
    ...
    const sess=[
      {id:'dead',name:'🌙 Dead Zone',start:2,end:6,q:'POOR',note:'Volume sangat sepi'},
      {id:'asia_open',name:'🌏 Asia Open',start:6,end:9,q:'MODERATE',note:'Volume mulai'},
      {id:'asia_peak',name:'🔥 Asia Peak',start:9,end:12,q:'GOOD',note:'Volume Asia bagus'},
      {id:'lunch',name:'⚠️ Lunch',start:12,end:15,q:'CAUTION',note:'Volume turun'},
      {id:'london',name:'🌍 London',start:15,end:18,q:'PRIME',note:'Volume institusional!'},
      {id:'ny_pre',name:'🏗️ NY Pre',start:18,end:21,q:'BUILDING',note:'Pre-market AS.'},
      {id:'ny_open',name:'🚀 NY Open',start:21,end:23,q:'PRIME',note:'PRIME: Volume tertinggi!'},
      {id:'ny_late',name:'🌙 NY Late',start:23,end:2,q:'GOOD',note:'Volume masih bagus'},
    ];
    ...
    const out={
      ...
      fg,fgLabel,
      marketCharacter:{type:mcType,color:mcColor,desc:mcDesc,stats:{oversold:osCount,overbought:obCount,bullish:Math.round(bullPct/totC*100),bearish:Math.round((totC-bullPct)/totC*100)},strategy:mcStrat,risk:mcRisk,positionSize:mcPos,marketPct:Math.round(bullPct/totC*100)+'% bullish'},
      btcSnapshot:{price:btcP,ch24:btcC,rsi:btcK?.rsi||null,rsiSlope:btcK?.rsiSlope||'—',volTrend:btcK?.volTrend||'—',fg,fgLabel,macd:btcK?.macd||null,atr:btcATRval,atrUsd:btcATRusd,resistance:btcRes,support:btcSup,current:btcP,aboveEma200:btcK?.aboveE200||false,btcLongPct:btcL,btcShortPct:btcS},
      ...
      gamePlan:{btcLevels:{resistance:btcRes,support:btcSup,current:btcP},scenarios:{bull:{condition:'BTC tembus $'+btcRes+' close di atas',action:'Long conv≥65 RR1:3 RS+FR filter',setups:top3},sideways:{condition:'BTC konsolidasi ±1.5%',action:'Scalp COILING+ACCUM saja.'},bear:{condition:'BTC breakdown ke $'+bearTgt,action:'Cash 80%. SHORT RSI>72.'}},spotAccum:longs.filter(x=>x.rsi<35).slice(0,8).map(x=>({sym:x.sym,price:x.price,rsi:x.rsi,signal:x.signal||'SIDEWAYS',dcaPrice:+(x.price*0.975).toFixed(x.price>1?4:8)})),avoidList:coins.filter(x=>x.rsi>80).slice(0,5).map(x=>({sym:x.sym,rsi:x.rsi,pct:x.c24}))},
      sectorFlow:{sectors,sectorData},
      checklist:{checks,pass,total:8,overallGreenLight:pass>=6,verdict:pass>=6?'✅ KONDISI LAYAK TRADING':'⚠️ HATI-HATI — '+(8-pass)+' kondisi belum terpenuhi',manualItems:['RSI koin < 72','Conv Score ≥ 60','FR < +0.04%','RR min 1:2','No entry 30min sebelum news','Vol ≥ $5M','Size ≤ 2% equity','SL ATR-based','Volume konfirmasi','Sesuai skenario Game Plan'],overallGreenLight:pass>=6},
      tradingSchedule:{wibHour:wibH,dayName:days[now.getUTCDay()],sessions:sess,currentSession:cs,position:cso.q,currentSessionObj:cso,nextPrime:nxt},
