// api/altcoins.js — v19
// PRIMARY: CoinGecko markets (sama dengan scanner, RELIABLE)
// RSI: CryptoCompare 4H aggregate (terbukti bekerja)

const STABLES = new Set(['tether','usd-coin','binance-usd','dai','trueusd','first-digital-usd','usdd','frax','usdb','usdb','stasis-eurs']);
const SECTORS = {
  BTC:'BTC',ETH:'ETH',BNB:'BNB',SOL:'L1',ADA:'L1',AVAX:'L1',DOT:'L1',NEAR:'L1',APT:'L1',SUI:'L1',SEI:'L1',INJ:'L1',TIA:'L1',TON:'L1',ATOM:'IOP',
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const src     = markets.length > 10 ? 'CoinGecko' : 'unavailable';

    if (!markets.length) {
      return res.status(200).json({ok:false,error:'CoinGecko unavailable',ts:Date.now(),elapsed:Date.now()-t0,version:'v19',fg:50,src:'unavailable',realRSICount:0,totalCoins:0,gainers:[],losers:[],rsiList:[],volBreakouts:[],topGainers:[],topLosers:[],rsiExtremes:[],volumeBreakouts:[],market:{signal:'UNKNOWN',avg24h:0,total:0,pos:0,neg:0,bigMove:0,totalVol:0}});
    }

    // Filter stables
    const btcRef = markets.find(c=>c.id==='bitcoin');
    const btcCh  = btcRef?.price_change_percentage_24h || 0;
    const filtered = markets.filter(c => !STABLES.has(c.id) && c.current_price > 0 && (c.total_volume||0) > 100000);
    ...
    const all = filtered.map(c => {
      ...
      // stablecoin filter by price
      if (price>=0.97&&price<=1.03&&Math.abs(ch24)<0.5) return null;
      const rp  = high>low?(price-low)/(high-low):0.5;
      const body= high>low?Math.abs(price-(c.current_price/((1+ch24/100)||1)))/(high-low):0;
      const vt  = vol>=1e9?5:vol>=200e6?4:vol>=50e6?3:vol>=10e6?2:vol>=1e6?1:0;
      return {
        symbol:sym, id:c.id, price:fmtP(price), ch24:+ch24.toFixed(2),
        vol:+vol.toFixed(0), high, low, body:+Math.min(1,Math.abs(body)).toFixed(3),
        rp:+rp.toFixed(3), vt,
        sector:SECTORS[sym]||null, rsi:50, rsiReal:false,
        mcap:c.market_cap||0, mcapRank:c.market_cap_rank||999,
      };
    }).filter(Boolean);
    ...
    const volBreakouts = all.filter(c=>c.vol>=8e6).map(c=>{
      ...
      else if (c.vt>=3&&c.ch24>5&&c.body>0.4){sc=93;bType='BREAKOUT 🚀';}
    ...
    // ── STEP 2: RSI via CryptoCompare 4H ─────────────────
    const rsiSyms = all.slice(0,22).map(c=>c.symbol);
    let realRSICount=0;
    ...
    ccR.forEach((r,i)=>{
      ...
      const closes=raw.map(k=>+k.close).filter(v=>v>0);if(closes.length<16)return;
    ...
    all.filter(c=>!c.rsiReal).forEach(c=>{
      const base=50,chC=Math.max(-25,Math.min(25,c.ch24*2));
      const rpC=Math.max(-15,Math.min(15,(c.rp-0.5)*30));
    ...
    return res.status(200).json({ok:false,error:e.message,ts:Date.now(),elapsed:Date.now()-t0,version:'v19',fg:50,src:'error',realRSICount:0,totalCoins:0,gainers:[],losers:[],rsiList:[],volBreakouts:[],topGainers:[],topLosers:[],rsiExtremes:[],volumeBreakouts:[],market:{signal:'UNKNOWN',avg24h:0,total:0,pos:0,neg:0,bigMove:0,totalVol:0}});
  }
}
