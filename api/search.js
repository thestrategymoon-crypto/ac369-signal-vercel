// api/search.js - AC369 FUSION Search & Deep Analysis (Fixed)
export default async function handler(req, res) {
  ...
  try {
    ...
    // 2. Jika CoinGecko gagal, coba ambil harga dari Binance
    if (currentPrice === 0) {
      try {
        const binanceTicker = await fetch(`https://api.binance.com/api/v3/ticker/24hr?symbol=${symbol}USDT`);
        if (binanceTicker.ok) {
          const ticker = await binanceTicker.json();
    ...
    // Jika tetap 0, koin tidak ditemukan
    if (currentPrice === 0) {
      return res.status(404).json({ error: `Koin "${symbol}" tidak ditemukan di CoinGecko maupun Binance.` });
    }

    // 3. Ambil OHLCV dari Binance
    let ohlcv = [];
    try {
      const binanceKlines = await fetch(`https://api.binance.com/api/v3/klines?symbol=${symbol}USDT&interval=1d&limit=100`);
      if (binanceKlines.ok) {
        const klines = await binanceKlines.json();
        ohlcv = klines.map(c => ({
      ...
      console.warn('Binance klines gagal:', e.message);
    }

    // 4. Jalankan semua analisis
    ...
    if (ohlcv.length >= 20) {
      ...
      else if (lastVol > avgVol * 1.5) volumeNote = `📈 Di atas rata-rata`;
    }
...
function detectElliottWave(ohlcv, change24h) {
  if (!ohlcv || ohlcv.length < 20) return { wave: 'Data terbatas', confidence: 20, description: '' };
  const swings = findSwingPoints(ohlcv, 3);
  if (swings.length < 3) return { wave: 'Struktur belum jelas', confidence: 25, description: '' };
  const recent = swings.slice(-5);
  const highs = recent.filter(s => s.type === 'high');
  const lows = recent.filter(s => s.type === 'low');
  if (highs.length < 2 || lows.length < 2) return { wave: 'Menunggu formasi', confidence: 30, description: '' };
...
function analyzeSMC(ohlcv, change24h) {
  if (!ohlcv || ohlcv.length < 10) return { signal: 'Neutral', summary: 'Data tidak cukup' };
  ...
  const recent = ohlcv.slice(-5), closes = recent.map(c => c.close);
  if (closes.length >= 2) {
