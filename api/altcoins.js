// api/altcoins.js — AC369 FUSION v14
// Fast parallel RSI batch, dynamic gainers threshold, sector tags

const STABLES = new Set(['USDT','USDC','BUSD','DAI','TUSD','USDP','FDUSD','USDD','GUSD','FRAX','LUSD','EURC','EUR','GBP','TRY','BRL']);
const IGNORE  = new Set(['BTCDOMUSDT','DEFIUSDT','USDCUSDT','PERPUSDT']);
const BAD_SFX = ['UP','DOWN','BULL','BEAR','LONG','SHORT','3L','3S','5L','5S'];
const SECTORS = {
  BTC:'BTC',ETH:'ETH',BNB:'BNB',SOL:'L1',ADA:'L1',AVAX:'L1',DOT:'L1',NEAR:'L1',APT:'L1',SUI:'L1',SEI:'L1',INJ:'L1',TIA:'L1',
  ATOM:'IOP',OSMO:'IOP',
...
function buildCoin(t) {
  ...
  const high = +(t.highPrice || price*1.01), low = +(t.lowPrice || price*0.99), open = +(t.openPrice || price);
  ...
  return { symbol:sym, price:+price.toFixed(dp), ch24:+ch24.toFixed(2), vol:+vol.toFixed(0), high:+high.toFixed(dp), low:+low.toFixed(dp), range:+(range*100).toFixed(2), rp:+rp.toFixed(3), body:+body.toFixed(3), vt, sector:SECTORS[sym]||null, rsi:50, rsiReal:false };
}
...
export default async function handler(req, res) {
  ...
  const sf = async (url, ms=6000) => {
    ...
    try { const r = await fetch(url,{signal:c.signal,headers:{'User-Agent':'AC369/1.0'}}); clearTimeout(t); if(!r.ok) return null; return await r.json(); }
  ...
  try {
    ...
    const [tickerR, fngR] = await Promise.allSettled([
      (async()=>{
        ...
        if (by?.result?.list?.length > 50) return { data: by.result.list.map(t=>({
    ...
    if (!rawTickers.length) return res.status(200).json({ timestamp:Date.now(), src:'unavailable', fg, version:'v14', gainers:[], losers:[], rsiList:[], volBreakouts:[], topGainers:[], topLosers:[], rsiExtremes:[], volumeBreakouts:[], market:{signal:'UNKNOWN',avg24h:0,total:0,pos:0,neg:0,bigMove:0,totalVol:0}, realRSICount:0 });

    // STEP 2: Filter & build
    const all = rawTickers.filter(t=>{
      if (!t?.symbol?.endsWith('USDT')) return false;
      if (IGNORE.has(t.symbol)) return false;
      const b = t.symbol.replace('USDT','');
      if (STABLES.has(b)) return false;
      if (BAD_SFX.some(p=>b.endsWith(p)||b.startsWith(p))) return false;
    ...
    // STEP 4: Gainers (dynamic threshold)
    const dynThresh = Math.max(0.5, avg24h + 1.5);
    let gainers = all.filter(c=>c.ch24>=dynThresh&&c.vol>=300000)
    ...
    const MAJOR = ['BTCUSDT','ETHUSDT','BNBUSDT','SOLUSDT','XRPUSDT','ADAUSDT','AVAXUSDT','DOGEUSDT','LINKUSDT','DOTUSDT','MATICUSDT','NEARUSDT','APTUSDT','INJUSDT','ARBUSDT','OPUSDT','SUIUSDT','TIAUSDT','FETUSDT','RENDERUSDT'];
    const top10 = all.filter(c=>!MAJOR.includes(c.symbol+'USDT')).sort((a,b)=>b.vol-a.vol).slice(0,10).map(c=>c.symbol+'USDT');
    ...
    return res.status(200).json({
      ...
      src, fg, realRSICount, version:'v14',
    ...
    return res.status(200).json({ timestamp:Date.now(), src:'error', error:e.message, fg:50, version:'v14', gainers:[], losers:[], rsiList:[], volBreakouts:[], topGainers:[], topLosers:[], rsiExtremes:[], volumeBreakouts:[], market:{signal:'UNKNOWN',avg24h:0,total:0,pos:0,neg:0,bigMove:0,totalVol:0}, realRSICount:0 });
  }
}
