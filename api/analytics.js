...
async function fetchTicker(sym) {
  // 1. Binance Futures
  const f = await sf(`https://fapi.binance.com/fapi/v1/ticker/24hr?symbol=${sym}USDT`);
  if (f && !f.code && +f.lastPrice > 0) return { price: +f.lastPrice, change24h: +f.priceChangePercent, vol: +f.quoteVolume, src: 'binance_futures' };

  // 2. Binance Spot
  const s = await sf(`https://api.binance.com/api/v3/ticker/24hr?symbol=${sym}USDT`);
  if (s && !s.code && +s.lastPrice > 0) return { price: +s.lastPrice, change24h: +s.priceChangePercent, vol: +s.quoteVolume, src: 'binance_spot' };

  // 3. CryptoCompare (reliable, always works)
  const cc = await sf(`https://min-api.cryptocompare.com/data/pricemultifull?fsyms=${sym}&tsyms=USD`);
  if (cc?.RAW?.[sym]?.USD?.PRICE > 0) {
    const d = cc.RAW[sym].USD;
    return { price: d.PRICE, change24h: d.CHANGEPCT24HOUR || 0, vol: d.TOTALVOLUME24HTO || 0, src: 'cryptocompare' };
  }

  // 4. CoinGecko
  const cgId = { BTC: 'bitcoin', ETH: 'ethereum' }[sym];
  if (cgId) {
    const cg = await sf(`https://api.coingecko.com/api/v3/simple/price?ids=${cgId}&vs_currencies=usd&include_24hr_change=true&include_24hr_vol=true`);
    if (cg?.[cgId]?.usd > 0) return { price: cg[cgId].usd, change24h: cg[cgId].usd_24h_change || 0, vol: cg[cgId].usd_24h_vol || 0, src: 'coingecko' };
  }

  // 5. CoinCap
  const capId = { BTC: 'bitcoin', ETH: 'ethereum' }[sym];
  if (capId) {
    const cap = await sf(`https://api.coincap.io/v2/assets/${capId}`);
    if (cap?.data?.priceUsd > 0) return { price: +cap.data.priceUsd, change24h: +cap.data.changePercent24Hr || 0, vol: +cap.data.volumeUsd24Hr || 0, src: 'coincap' };
  }
  return null;
}
