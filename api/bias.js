  ...
  try {
    ...
    const [fngRes, btcKlines4h, globalRes, tickerRes] = await Promise.allSettled([
      ...
      sf('https://min-api.cryptocompare.com/data/v2/histohour?fsym=BTC&tsym=USD&limit=200')
        .then(d => d?.Response === 'Success' && d.Data?.Data?.length > 20 ? d.Data.Data.map(x => +x.close).filter(v => v > 0) :
          sf('https://fapi.binance.com/fapi/v1/klines?symbol=BTCUSDT&interval=4h&limit=50')
            .then(d => Array.isArray(d) && d.length > 10 ? d.map(k => +k[4]) :
              sf('https://api.binance.com/api/v3/klines?symbol=BTCUSDT&interval=4h&limit=50')
                .then(d => Array.isArray(d) ? d.map(k => +k[4]) : null)
            )
        ),
    ...
    return res.status(200).json({
      ...
      details: biasDetails,
      recommendation,
      astro: { moonPhase, moonEmoji, halvingPhase, daysSinceHalving, inMercuryRetrograde: inMR, inMercuryShadow, mercuryWarning, daysSinceNM: +dnm.toFixed(1) },
    });
  } catch (e) {
