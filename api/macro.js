// api/macro.js — AC369 FUSION v10.3
// FIXED: BTC klines limit 200 for MVRV calculation
// FIXED: All response fields match index.html:
//   data.fearGreed.value
//   data.btcDominance.dominance / .interpretation
//   data.mvrvZScore.value / .interpretation
//   data.altcoinSeason.index / .season
//   data.cycleSummary
...
export default async function handler(req, res) {
  ...
  try {
    const [fngRes, globalRes, btcKlinesRes, tickerRes, trendingRes] = await Promise.allSettled([
      ...
      // CRITICAL: limit=200 for proper MVRV calculation
    ...
    if (fngRes.status==='fulfilled'&&fngRes.value?.data) {
      ...
      fearGreed = {
        ...
        history:d.slice(0,7).map(x=>({value:parseInt(x.value),label:x.value_classification,
    ...
    if (tickerRes.status==='fulfilled'&&Array.isArray(tickerRes.value)) {
      const tMap={};
      tickerRes.value.forEach(t=>{tMap[t.symbol]=t;});
      ...
      let out=0,tot=0;
      ALTS.forEach(sym=>{
        if(tMap[sym]){tot++;if(parseFloat(tMap[sym].priceChangePercent)>btcChg)out++;}
      });
    ...
    if (btcKlinesRes.status==='fulfilled'&&Array.isArray(btcKlinesRes.value)&&btcKlinesRes.value.length>=90) {
      ...
      mvrvZScore={value:z.toString(),estimate:z,interpretation:interp,
    ...
    let topGainers=[],topLosers=[],volumeBreakout=[],trendingCoins=[];
    if (tickerRes.status==='fulfilled'&&Array.isArray(tickerRes.value)) {
      const STABLES=new Set(['USDT','USDC','BUSD','TUSD','DAI','FDUSD']);
      const f=tickerRes.value.filter(t=>t&&t.symbol&&t.symbol.endsWith('USDT')&&parseFloat(t.quoteVolume||0)>5000000&&!STABLES.has(t.symbol.replace('USDT','')));
      ...
      volumeBreakout=[...f].filter(t=>parseFloat(t.quoteVolume)>50000000).sort((a,b)=>parseFloat(b.quoteVolume)-parseFloat(a.quoteVolume)).slice(0,8).map(t=>({...mp(t),signal:parseFloat(t.priceChangePercent)>3?'BULLISH_BREAKOUT':parseFloat(t.priceChangePercent)<-3?'BEARISH_BREAKDOWN':'HIGH_VOLUME'}));
    }
    if (trendingRes.status==='fulfilled'&&trendingRes.value?.coins) {
      trendingCoins=trendingRes.value.coins.slice(0,7).map(c=>({name:c.item.name,symbol:c.item.symbol,rank:c.item.market_cap_rank,score:c.item.score}));
    }
