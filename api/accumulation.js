// api/accumulation.js — AC369 FUSION v13.0
// ══════════════════════════════════════════════════════════════════
// SMART MONEY DAY TRADE & SCALP SCANNER
// Guaranteed results — always surfaces best opportunities
//
// Strategy:
//   1. Scan ALL Binance pairs by volume
//   2. Score each coin across 6 dimensions
//   3. Output top setups with precise ICT Entry/SL/TP
//   4. Trade style: SCALP (fast) / DAY TRADE / SWING
// ══════════════════════════════════════════════════════════════════
...
// ── NARRATIVE MAP ─────────────────────────────────────────────────
const NARRATIVES = {
  IOTX:'DePIN', HNT:'DePIN', FIL:'DePIN', AR:'DePIN', STORJ:'DePIN',
  ONDO:'RWA', POLYX:'RWA', CFG:'RWA', OM:'RWA',
  VITA:'DeSci', BIO:'DeSci', RSC:'DeSci',
  ...
  ZEC:'Privacy', ROSE:'Privacy',
};

const STABLES = new Set(['USDT','USDC','BUSD','TUSD','DAI','FDUSD','USDP','FRAX','LUSD','USDS','SUSD','GUSD','PYUSD','CRVUSD','USDD','PAXG','XAUT','WBTC','WETH','WBNB','STETH','CBETH','RETH','AUSD','IDRT']);
...
export default async function handler(req, res) {
  ...
  // POST: ignored (no more watchlist sync complexity)
  if (req.method === 'POST') {
  ...
  function getKillZone() {
    ...
    if (t >= 1380 && t < 1560)return { zone: 'NY_CLOSE',     q: 'ACTIVE',  b: 2, e: '🔔' };
  ...
  // Returns score 0-100 for any coin
  function scoreCoin(coin, btcCh24, btcCh7, fg, hv) {
    const { ch24, ch7, vol, high, low, open, price, ath, mcap } = coin;
    if (!price || price <= 0) return null;

    const range = Math.max(high - low, price * 0.005);
    const rp   = (price - low) / range;           // 0=at low, 1=at high
    const lw   = (Math.min(price, open) - low) / range; // lower wick ratio
    const body = Math.abs(price - open) / range;  // body size
    const vt   = vol >= 500e6 ? 5 : vol >= 100e6 ? 4 : vol >= 30e6 ? 3 : vol >= 10e6 ? 2 : vol >= 2e6 ? 1 : 0;

    if (vt === 0) return null; // require minimum $2M volume
    if (ch24 < -20) return null; // skip crashed coins
    if (ch24 > 50)  return null; // skip parabolic pumps

    const fromATH = ath > 0 ? (price - ath) / ath * 100 : -50;
    const rs7     = (ch7 || 0) - (btcCh7 || 0); // relative to BTC
    ...
    // ── 1. MOMENTUM (0-25) ─────────────────────────────────────
    let mom = 0;
    if (rs7 > 15)     { mom = 25; signals.push('RS⬆⬆'); reasons.push(`+${rs7.toFixed(0)}% outperform BTC`); }
    else if (rs7 > 8) { mom = 20; signals.push('RS⬆');  reasons.push(`+${rs7.toFixed(0)}% vs BTC`); }
    else if (rs7 > 3) { mom = 16; }
    else if (rs7 > 0) { mom = 12; }
    else if (rs7 > -5){ mom = 8;  }
    else if (rs7 > -10){ mom = 5; }
    else               { mom = 3; }
    ...
    let vol_s = 0;
    const volM = vol / 1e6;
    if      (volM >= 500) { vol_s = 20; signals.push('VOL★★'); reasons.push(`$${volM.toFixed(0)}M inst. volume`); }
    else if (volM >= 100) { vol_s = 16; signals.push('VOL★');  reasons.push(`$${volM.toFixed(0)}M volume`); }
    else if (volM >= 30)  { vol_s = 12; }
    else if (volM >= 10)  { vol_s = 8; }
    else if (volM >= 2)   { vol_s = 5; }
    // Volume spike: high vol + controlled move = accumulation
    if (volM >= 30 && Math.abs(ch24) <= 5) {
      vol_s = Math.min(20, vol_s + 4);
      signals.push('ACCUM');
      reasons.push(`High vol + flat = institutional absorption`);
    }
    score += vol_s;

    // ── 3. PRICE STRUCTURE (0-20) ─────────────────────────────
    let struct = 0;
    // Liquidity sweep (wick below lows)
    if (lw > 0.40 && rp < 0.55 && price > low * 1.001) {
      struct += 12;
      signals.push('LIQ.SWEEP');
      reasons.push(`SSL swept: wick ${(lw*100).toFixed(0)}% below — stops cleared`);
    }
    // BOS: price in upper range after down period
    ...
    if (rp < 0.40) {
      struct += 6;
      signals.push('DISCOUNT');
      reasons.push(`${(rp*100).toFixed(0)}% of range — discount zone`);
    } else if (rp < 0.55) {
      struct += 3;
    }
    // Rejection candle (small body + lower wick)
    if (body < 0.30 && lw > 0.20) {
      struct += 4;
      signals.push('REJECTION');
    }
    score += Math.min(20, struct);

    // ── 4. ATH DISTANCE / CYCLE POSITION (0-15) ──────────────
    let ath_s = 0;
    const inGolden = fromATH <= -55 && fromATH >= -85;
    const inEarly  = fromATH <= -30 && fromATH >= -55;
    if (inGolden)      { ath_s = 15; signals.push('GOLDEN'); reasons.push(`${fromATH.toFixed(0)}% from ATH — golden accumulation zone`); }
    else if (inEarly)  { ath_s = 10; signals.push('EARLY');  reasons.push(`${fromATH.toFixed(0)}% from ATH — early entry zone`); }
    else if (fromATH > -30) { ath_s = 5; }
    else               { ath_s = 3; }
    // Halving cycle bonus
    ath_s = Math.min(15, ath_s + Math.max(0, hv.b * 0.3));
    score += ath_s;

    // ── 5. NARRATIVE / SECTOR (0-10) ─────────────────────────
    const sym = coin.base;
    const narr = NARRATIVES[sym] || null;
    let narr_s = 0;
    if (narr) {
      const trendingNarr = new Set(['AI','DePIN','RWA','DeSci','L2','Modular']);
      narr_s = trendingNarr.has(narr) ? 10 : 5;
      signals.push(narr);
      if (narr_s >= 10) reasons.push(`${narr} — trending sector`);
    }
    score += narr_s;

    // ── 6. MARKET ALIGNMENT (0-10) ───────────────────────────
    let mkt_s = 0;
    if (fg <= 20)  { mkt_s = 10; signals.push('EXT.FEAR'); }
    else if (fg <= 35) { mkt_s = 7; }
    else if (fg >= 75) { mkt_s = 2; }
    else { mkt_s = 5; }
    if (btcCh7 > 3) mkt_s = Math.min(10, mkt_s + 3);
    score += mkt_s;

    // ── COMPUTE OTE + TRADE LEVELS ────────────────────────────
    const range_abs = high - low;
    const slPct  = Math.max(0.04, Math.min(0.12, range_abs / price * 1.5 + 0.02));
    const sl     = +(price * (1 - slPct)).toFixed(8);
    const slDist = price - sl;

    // OTE: Fibonacci 62-79% of swing
    const fib618 = +(low + range_abs * 0.618).toFixed(8);
    const fib786 = +(low + range_abs * 0.786).toFixed(8);
    const inOTE  = price >= fib618 * 0.998 && price <= fib786 * 1.002;

    // Entry: current price (or OTE level if below)
    const entry = inOTE ? +((fib618 + fib786) / 2).toFixed(8) : +(price * 0.999).toFixed(8);
    const entryLo = +(entry * 0.997).toFixed(8);
    const entryHi = +(entry * 1.003).toFixed(8);

    // TPs: 1:2, 1:3, 1:4.5
    const tp1 = +(entry + slDist * 2).toFixed(8);
    const tp2 = +(entry + slDist * 3).toFixed(8);
    const tp3 = +(entry + slDist * 4.5).toFixed(8);
    const tp1Pct = +((tp1 - entry) / entry * 100).toFixed(1);
    const tp2Pct = +((tp2 - entry) / entry * 100).toFixed(1);
    const tp3Pct = +((tp3 - entry) / entry * 100).toFixed(1);
    ...
    // Probability (1-10)
    const confCount = signals.length;
    const probScore = Math.min(10, Math.max(1, Math.round(confCount * 1.5 + score / 15)));
  ...
  try {
    ...
    const btcCh7  = 0; // no 7d from 24hr endpoint - use ch24 proxy
    ...
    coins.forEach(coin => {
      ...
      if (inZone && probScore >= 7) {
        decision = '⚡ EXECUTE — IN ZONE'; decisionColor = '#ff6b35';
      } else if (inZone) {
        decision = '✅ LIMIT ENTRY'; decisionColor = '#00ffd0';
      } else {
        decision = `🎯 SET LIMIT — $${fmtP(trade.entryLo)}–$${fmtP(trade.entryHi)}`; decisionColor = '#00ffd0';
      }
      ...
      scored.push({
        rank: 0,
        symbol: coin.base, name: coin.name || coin.base,
        assetType: coin.vol >= 200e6 ? 'MAJOR' : coin.vol >= 30e6 ? 'ALT' : 'SMALL',
        isMeme: false,
        price: coin.price, ch24: coin.ch24, ch7: coin.ch7 || 0, vol: coin.vol,
        fromATH,
        ...
        preImpulse: { count: signals.length, stage: signals.length >= 4 ? 'READY 🔥' : signals.length >= 3 ? 'STRONG 💪' : 'FORMING 🔄', sigs: reasons.map(r => ({ s: r, note: r })) },
        ...
        rr: {
          ...
          rr1: 2.0, rr2: 3.0,
          slPrice: trade.sl, slPct: trade.slPct,
          label: '1:2 / 1:3',
        },
        entryZone: { lo: trade.entryLo, hi: trade.entryHi, optimal: trade.entry },
        scoreBreakdown: { pi: Math.round(score * 0.25), sm: Math.round(score * 0.25), mom: Math.round(score * 0.20), str: Math.round(score * 0.15), mkt: Math.round(score * 0.15) },
        halvingPhase: hv.p, halvingEmoji: hv.e,
        moonPhase: 'N/A', moonEmoji: '🌙', moonLabel: '',
        decision, decisionColor, positionSize: posSize, isElite,
        rrMode: tradeStyle === 'SCALP' ? 'SCALP (1:2–1:3)' : 'DAY TRADE (1:2–1:4.5)',
        tradeStyle,
        probScore,
        ict: {
          liq: { signals: reasons.filter(r => r.includes('SSL') || r.includes('wick')).map(r => ({ type: 'SSL_SWEEP', note: r })), hasSweep: lw > 0.30, hasInducement: false, hasFVG: false },
          mss: { signals: reasons.filter(r => r.includes('BOS')).map(r => ({ type: 'BOS', note: r })), hasMSS: false, hasBOS: !!reasons.find(r => r.includes('BOS')), hasFVG: false, fvgZone: null, inDiscount: rp < 0.50 },
          ...
          entryPkg: {
            oteEntry: trade.entry, entryZone: { lo: trade.entryLo, hi: trade.entryHi },
            sl: trade.sl, slNote: `Below ${trade.slPct}% structural zone`,
            tp1: trade.tp1, tp1Pct: trade.tp1Pct, tp1Note: 'Partial 50% — 1:2',
            tp2: trade.tp2, tp2Pct: trade.tp2Pct, tp2Note: 'Full exit — 1:3',
        ...
        tradeData: {
          coin: coin.base, type: coin.vol >= 100e6 ? 'MAJOR' : 'ALT',
    ...
    // Sort by score desc, always return results
    ...
    // Regular = top setups regardless of style
    const regular  = scored.slice(0, 15);
    const elite    = scored.filter(r => r.isElite).slice(0, 5);

    // Telegram for top elite
    if (elite.length > 0 && elite[0]) {
      const t = elite[0];
      tg(`🏆 <b>TOP SETUP — ${t.symbol}</b>\nScore: ${t.finalScore} | Prob: ${t.probScore}/10 | ${t.tradeStyle}\n📍 Entry: $${fmtP(t.ict.entryPkg.oteEntry)}\n🛑 SL: $${fmtP(t.ict.entryPkg.sl)} (-${t.rr.slPct}%)\n🎯 TP1: $${fmtP(t.rr.tp1)} (+${t.rr.tp1Pct}%)\n${t.halvingEmoji} ${t.halvingPhase} · ${kz.e} ${kz.zone}`);
    }
    ...
    return res.status(200).json({
      ...
      cosmic: { halving: { phase: hv.p, bonus: hv.b, emoji: hv.e }, btcCh7 },
      regime: {
      ...
      systemStatus: { btcTrend: btcCh24 >= 0 ? 'BULLISH' : 'BEARISH', fgValue: fg, btcDom, btcCh7: 0, btcPx },
      killedBreakdown: { KILL: 0, PI: 0, SM: 0, RR: 0, CONFLICT: 0, SCORE: scored.length > 0 ? 0 : coins.length, STALE: 0 },
      totalKilled: 0,
    });

  } catch (e) {
    return res.status(500).json({ error: e.message, version: 'v13.0', totalScanned: 0, totalQualified: 0, regularSetups: [], memeSetups: [] });
  }
}
