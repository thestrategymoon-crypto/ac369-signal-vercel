// api/morning-brief.js — 369 GLOBAL CRYPTO v20.0
// ═══════════════════════════════════════════════════════════════════
// ROOT CAUSE FIXES dari v2 yang broken:
//
// BUG #1 — FR THRESHOLD SALAH (penyebab 0 LONG · 0 ACCUM · 0 FLY)
//   Bybit fundingRate field: 0.0001 = 0.01% per 8h (NEUTRAL market)
//   frPct = fundingRate × 100 = 0.01 untuk market normal
//   v2 pakai frPct<=0.001 → TIDAK PERNAH FIRE di market normal!
//   v20 FIX: FR_NEUTRAL = 0.01, semua threshold dikalibrasi ulang
//
// BUG #2 — KLINES TIMEOUT (penyebab 0 real RSI)
//   v2: 65 coin × 6000ms parallel → Vercel 10s limit = semua null
//   v20 FIX: 20 premium coins only, 4500ms timeout, guaranteed fit
//
// BUG #3 — CACHE BROKEN DATA
//   v2: cache tidak check realRSI → broken data di-serve 90s
//   v20 FIX: cache HANYA valid jika realRSI >= 10
//
// BUG #4 — OI DELTA SELALU 0
//   v2: oiChangePct hardcoded 0 → futures intelligence lemah
//   v20 FIX: OI delta tracking antar API call via persistent cache
// ═══════════════════════════════════════════════════════════════════
'use strict';

// ─── FR CALIBRATION (KRITIS) ──────────────────────────────────────
// Bybit default fundingRate = 0.0001 → frPct = 0.01%
// Semua threshold FR di bawah ini dalam unit frPct (%)
const FR_NEUTRAL   = 0.010; // 0.010% = normal Bybit rate, BUKAN tanda apapun
const FR_ELEVATED  = 0.040; // 0.040% = longs terlalu banyak, hati-hati
const FR_HOT       = 0.080; // 0.080% = overheated, dump risk
const FR_COOL      = 0.005; // 0.005% = di bawah neutral = mild bearish futures
const FR_NEG       = 0.000; // 0.000% = benar-benar negative = bearish sentiment
const FR_SQUEEZE   =-0.005; // -0.005% = squeeze setup signal
const FR_EXTREME   =-0.020; // -0.020% = extreme squeeze, sangat langka

// ─── LIQUIDITY GATES ─────────────────────────────────────────────
const MIN_VOL_MAP    =  200_000;  // masuk coin map
const MIN_VOL_SIG    =  500_000;  // dapat sinyal (0.5M USDT/hari)
const MIN_VOL_QUAL   = 2_000_000; // sinyal berkualitas (2M)
const MIN_VOL_ELITE  = 5_000_000; // setup ELITE (5M)
const MIN_OI_WATCH   = 3_000_000; // masuk watchlist
const MIN_OI_WHALE   = 50_000_000; // whale radar (50M OI)
const MIN_OI_FUT     = 20_000_000; // futures intel (20M OI)
const MIN_OI_FLOW    = 30_000_000; // sector flow (30M OI)

// ─── PREMIUM KLINE COINS (20 coins — guaranteed Bybit linear) ────
// Hanya coins ini dapat REAL 4H RSI, ATR, MACD, OBV
// Dikurangi dari 65 ke 20 untuk mencegah Vercel 10s timeout
const PREMIUM = [
  'BTC','ETH','SOL','BNB','XRP','ADA','AVAX','DOGE',
  'LINK','DOT','NEAR','SUI','ARB','OP','INJ',
  'PEPE','WIF','HYPE','TIA','ENA',
];

// ─── FILTERS ─────────────────────────────────────────────────────
const STAB = new Set([
  'USDT','USDC','BUSD','DAI','TUSD','FDUSD','USDE','FRAX',
  'USDP','SUSD','LUSD','PYUSD','EURC','USDD','BIDR','IDRT',
  'USDJ','USTC','GUSD','MUSD','USDX',
]);
...
const isBad = s => {
  ...
  if (BAD_SFX.some(b => s.endsWith(b))) return true;
  if (BAD_PFX.some(b => s.startsWith(b))) return true;
  return false;
};
...
const SECTORS = {
  Bitcoin:['BTC'],
  Ethereum:['ETH'],
  L1:['SOL','ADA','AVAX','TON','NEAR','SUI','APT','SEI','INJ','HBAR','ALGO','XLM','TRX','VET','ATOM','BNB','FTM','ONE'],
  L2:['ARB','OP','MATIC','STRK','IMX','ZK','SCROLL','BLAST','MANTA','METIS','BASE'],
  DeFi:['UNI','AAVE','CRV','MKR','SNX','COMP','PENDLE','GMX','JUP','DYDX','LDO','SUSHI','ENA','ONDO','CAKE'],
  Payments:['XRP','LTC','BCH','XLM','DASH','XMR'],
  AIDePin:['RENDER','FET','AGIX','TAO','WLD','IO','ARKM','VIRTUAL','OLAS','OCEAN','GRASS'],
  Infrastructure:['LINK','DOT','FIL','GRT','BAND','PYTH','JTO','W','EIGEN','TIA','API3'],
...
const sf = async (url, ms=5000) => {
  ...
  try {
    const r = await fetch(url, {
      signal: ctrl.signal,
      headers: { 'Accept':'application/json', 'User-Agent':'369Global/20.0' },
    });
    clearTimeout(tmr);
    if (!r.ok) return null;
    return await r.json();
...
// ─── TA FUNCTIONS ─────────────────────────────────────────────────
// Wilder RSI-14 (proper seed + smoothing)
function rsi14(cls) {
...
function macd(cls) {
  ...
  const n=mv.length,last=mv[n-1],prev=mv[n-2]||last;
...
function atr(K, p=14) {
  ...
  const trs=K.slice(1).map((k,i)=>Math.max(
    N(k.h)-N(k.l), Math.abs(N(k.h)-N(K[i].c)), Math.abs(N(k.l)-N(K[i].c))
  ));
  return trs.slice(-p).reduce((s,v)=>s+v,0)/p;
}

// Parse Bybit v5 kline response: newest-first → oldest-first
// Format: [openTime, open, high, low, close, volume(base), turnover(USDT)]
...
// ─── GENERATE SIGNAL ─────────────────────────────────────────────
// !! KRITIS: semua frPct threshold dikalibrasi ke FR_NEUTRAL=0.01% !!
function generateSignal(coin) {
  ...
  // Hard gate: volume minimum
  if (vol < MIN_VOL_SIG) return mk('','#4a5568','','WAIT',50,[]);

  let sig='',sc='#4a5568',desc='',dir='WAIT',prob=50,tags=[];

  // ── LONG SIGNALS ──────────────────────────────────────────────────
  // P1: Capitulation — sangat langka, max prob
  if (rsiReal && rsi<22 && frPct<FR_NEG && c24<-5 && vol>2e6) {
    sig='CAPITULATION BUY 🆘'; sc='#00ffd0'; dir='LONG'; prob=93;
    desc=`RSI REAL ${rsi.toFixed(0)} extreme + FR ${frPct.toFixed(4)}% + dump = BOTTOM LANGKA`;
    tags=['RARE','CAPITULATION'];

  // P2: About to Fly — real RSI oversold + coiling + squeeze
  } else if (rsiReal && rsi<28 && frPct<FR_COOL && isCoiling && vol>500000) {
    sig='ABOUT TO FLY 🚀'; sc='#ffd700'; dir='LONG'; prob=90;
    desc=`RSI REAL ${rsi.toFixed(0)} + FR ${frPct.toFixed(4)}% + coiling = POMPA SEGERA`;
    tags=['FLY','COILING','REAL'];

  // P3: Whale Fingerprint — OI massive + flat + FR below neutral
  } else if (oi>200e6 && frPct<FR_NEUTRAL && Math.abs(c24)<2 && vol>3e6) {
    sig='WHALE FINGERPRINT 🐳'; sc='#00d4ff'; dir='LONG'; prob=88;
    desc=`OI $${(oi/1e9).toFixed(2)}B + harga flat + FR ${frPct.toFixed(4)}% = INSTITUSI AKUMULASI`;
    tags=['WHALE','STEALTH'];

  // P4: Extreme Squeeze — FR sangat negatif
  } else if (frPct<FR_SQUEEZE && vol>1e6 && oi>MIN_OI_FUT) {
    sig='SQUEEZE INCOMING 💎'; sc='#ff6b9d'; dir='LONG'; prob=88;
    desc=`FR ${frPct.toFixed(4)}% EXTREME = shorts bayar longs tiap 8h, pompa tinggal trigger`;
    tags=['SQUEEZE','FR_EXTREME'];

  // P5: New Longs Entering — OI naik + harga naik
  } else if (oiChg>5 && c24>2 && rsi<70 && vol>3e6) {
    sig='NEW LONGS ENTERING ⚡'; sc='#00ff88'; dir='LONG'; prob=85;
    desc=`OI +${oiChg.toFixed(1)}% + harga +${c24.toFixed(1)}% = posisi long valid masuk`;
    tags=['NEW_LONGS'];

  // P6: Short Squeeze Setup — retail terlalu short
  } else if ((retailLong||50)<=35 && rsi<55 && vol>2e6 && oi>MIN_OI_FUT) {
    sig='SHORT SQUEEZE SETUP 🔥'; sc='#fb7185'; dir='LONG'; prob=86;
    desc=`${100-(retailLong||50)}% retail SHORT = squeeze akan terjadi`;
    tags=['SQUEEZE','RETAIL'];

  // P7: Deep Oversold Real — RSI real klines
  } else if (rsiReal && rsi<28 && frPct<FR_ELEVATED && vol>500000) {
    sig='DEEP OVERSOLD REAL 📉'; sc='#f87171'; dir='LONG'; prob=82;
    desc=`RSI REAL ${rsi.toFixed(0)} oversold + FR ${frPct.toFixed(4)}% = mean reversion`;
    tags=['OVERSOLD','REAL'];

  // P8: Whale Accumulation — OI besar + flat price + normal/low FR
  } else if (oi>100e6 && Math.abs(c24)<2 && vol>2e6 && frPct<FR_ELEVATED) {
    sig='SMART ACCUMULATION 🤫'; sc='#a78bfa'; dir='LONG'; prob=83;
    desc=`OI $${(oi/1e9).toFixed(2)}B + flat + FR ${frPct.toFixed(4)}% = institusi akumulasi`;
    tags=['ACCUM','WHALE'];

  // P9: Pre-Breakout Coil — ATR compressed + OI ada
    ...
    desc=`ATR ${atrPct.toFixed(2)}% compressed = energi terkumpul, breakout imminent`;
    tags=['COIL'];

  // P10: OBV Divergence — volume beli saat harga turun
  } else if (obvBull && c24<-0.5 && rsi<45 && vol>500000) {
    sig='OBV DIVERGENCE 📊'; sc='#6ee7b7'; dir='LONG'; prob=79;
    desc=`Volume beli dominan saat harga -${Math.abs(c24).toFixed(1)}% = SM akumulasi`;
    tags=['OBV_DIV'];

  // P11: Narrative Momentum — outperform BTC significant
  } else if (rs>5 && c24>(btcC||0)+4 && vol>2e6 && rsi<72) {
    sig='NARRATIVE MOMENTUM 📡'; sc='#c084fc'; dir='LONG'; prob=81;
    desc=`RS +${rs.toFixed(1)}% vs BTC = catalyst/narrative aktif`;
    tags=['RS_STRONG'];

  // P12: Momentum Breakout — c24 kuat + RSI sehat
  } else if (c24>3 && rsi>=42 && rsi<=67 && vol>2e6 && frPct<FR_ELEVATED) {
    sig='MOMENTUM BREAKOUT 🚀'; sc='#22c55e'; dir='LONG'; prob=78;
    desc=`+${c24.toFixed(1)}% + RSI ${rsi.toFixed(0)} sehat + FR ok`;
    tags=['MOMENTUM'];

  // P13: Deep Oversold Estimated — rsi < 30 dari estimasi
  ...
  // P14: Mild Bull — kondisi standar layak trading
  // !! KRITIS: frPct < FR_ELEVATED (0.04), BUKAN <= 0.001 !!
  } else if (rsi>=38 && rsi<=63 && c24>0.2 && frPct<FR_ELEVATED && vol>1e6) {
    sig='MILD BULL 📈'; sc='#86efac'; dir='LONG'; prob=63;
    desc=`RSI ${rsi.toFixed(0)} + +${c24.toFixed(1)}% + FR ${frPct.toFixed(4)}% ok`;
    tags=['MILD'];
  ...
  // P15: Blow-off Top
    ...
    desc=`RSI ${rsi.toFixed(0)} + FR +${frPct.toFixed(4)}% + pump ${c24.toFixed(1)}% = DISTRIBUSI`;
    tags=['TOP','DIST'];

  // P16: Bull Trap
  } else if (rsi>73 && frPct>FR_ELEVATED && c24>4 && vol>2e6) {
    sig='BULL TRAP ⚠️'; sc='#ef4444'; dir='SHORT'; prob=73;
    desc=`RSI ${rsi.toFixed(0)} OB + FR +${frPct.toFixed(4)}% = SM jual ke retail`;
    tags=['OVERBOUGHT'];
...
function convScore(coin) {
  ...
  // Base 42 — ditingkatkan jika kondisi bagus
  let raw = 42;
  ...
  // RSI — real RSI dapat bonus
  const rB = rsiReal?3:0;
  raw += rB + (rsi<22?16:rsi<28?11:rsi<35?7:rsi<42?3:rsi<55?0:rsi>80?-18:rsi>73?-11:rsi>65?-5:0);

  // Funding rate — DIKALIBRASI BENAR
  // frPct normal = 0.01%, bukan 0%
  raw += frPct<FR_EXTREME?32:frPct<FR_SQUEEZE?23:frPct<FR_NEG?15:frPct<FR_COOL?9:
         frPct<FR_NEUTRAL?5:frPct<FR_ELEVATED?1:frPct<FR_HOT?-6:frPct>=FR_HOT?-20:0;
...
function detectWhale(coin) {
  ...
  // 1. Stealth accumulation: OI massive + flat + FR below neutral
  if (oi>200e6 && frPct<FR_NEUTRAL && Math.abs(c24)<2 && vol>2e6) {
    score+=28;
    sigs.push(`OI $${(oi/1e9).toFixed(2)}B + FR ${frPct.toFixed(4)}% + flat = STEALTH ACCUM`);
    tags.push('STEALTH');
  } else if (oi>50e6 && frPct<FR_NEUTRAL && Math.abs(c24)<3 && vol>1e6) {
    score+=16;
    sigs.push(`OI $${(oi/1e6).toFixed(0)}M + FR ${frPct.toFixed(4)}% = SM kumpul`);
    tags.push('ACCUM');
  }

  // 2. Volume absorption: vol besar tapi harga tidak naik
  const volOI = oi>0 ? vol/oi : 0;
  if (vol>20e6 && Math.abs(c24)<2 && volOI>0.1 && oi>30e6) {
    score+=20; sigs.push(`Vol $${(vol/1e6).toFixed(0)}M vs OI = ABSORPTION`); tags.push('ABSORB');
  } else if (vol>5e6 && Math.abs(c24)<1.5 && volOI>0.06) {
    score+=10; sigs.push(`Vol/OI ${(volOI*100).toFixed(1)}% = volume tanpa movement`);
  }

  // 3. OBV divergence: volume beli saat harga turun
  if (obvBull && c24<-1 && rsi<45) {
    score+=18; sigs.push(`OBV bullish saat harga -${Math.abs(c24).toFixed(1)}% = hidden accum`); tags.push('OBV');
  } else if (obvBull && c24<0) { score+=8; }

  // 4. FR squeeze (shorts membayar longs)
  if (frPct<FR_SQUEEZE) {
    score+=28; sigs.push(`FR ${frPct.toFixed(4)}% = shorts bayar longs mahal, pompa akan datang`); tags.push('SQUEEZE');
  } else if (frPct<FR_NEG) {
    score+=15; sigs.push(`FR ${frPct.toFixed(4)}% negatif = bearish futures = contrarian buy`); tags.push('NEG_FR');
  } else if (frPct<FR_COOL) {
    score+=7; sigs.push(`FR ${frPct.toFixed(4)}% di bawah neutral = mild bullish bias`);
  }

  // 5. RSI oversold + OI besar
  if (rsi<28 && oi>100e6) {
    score+=22; sigs.push(`RSI ${rsi.toFixed(0)} oversold + OI $${(oi/1e6).toFixed(0)}M = BOTTOM`); tags.push('BOTTOM');
  } else if (rsi<35 && oi>30e6) {
    score+=12; sigs.push(`RSI ${rsi.toFixed(0)} + OI $${(oi/1e6).toFixed(0)}M`);
  }

  // 6. Discount zone (harga di bawah range)
  if (pip<22 && oi>100e6) {
    score+=14; sigs.push(`Pip ${pip.toFixed(0)}% = harga di discount zone`); tags.push('DISCOUNT');
  }

  // 7. Coiling (ATR menyempit)
  if (isCoiling && atrPct>0 && atrPct<2.5 && oi>80e6) {
    score+=16; sigs.push(`ATR ${atrPct.toFixed(2)}% compressed = energi terkumpul`); tags.push('COIL');
  }
...
function analyzeFut(coin) {
  ...
  // 4 OI Direction States
  if (oiChg>3&&c24>1) {
    state='NEW LONGS'; stateColor='green'; score+=16;
    sigs.push(`OI +${oiChg.toFixed(1)}% + harga naik = long baru valid`); opp='LONG_ENTRY';
  } else if (oiChg>3&&c24<-1) {
    state='NEW SHORTS'; stateColor='red'; score-=12;
    sigs.push(`OI +${oiChg.toFixed(1)}% + harga turun = short baru dominan`); opp='WAIT';
  } else if (oiChg<-3&&c24>1) {
    state='SHORT SQUEEZE'; stateColor='cyan'; score+=14;
    sigs.push(`OI -${Math.abs(oiChg).toFixed(1)}% + harga naik = short cover paksa`); opp='MOMENTUM';
  } else if (oiChg<-3&&c24<-1) {
    state='LONG LIQ'; stateColor='orange'; score-=5;
    sigs.push(`OI -${Math.abs(oiChg).toFixed(1)}% + harga turun = long liquidasi`);
    opp = rsi<30?'DCA_ZONE':'WAIT';
  }

  // FR scoring (DIKALIBRASI BENAR)
  if (frPct<FR_EXTREME) {
    score+=30; sigs.push(`FR ${frPct.toFixed(4)}% EXTREME = SQUEEZE IMMINENT`); opp='SQUEEZE_PLAY';
  } else if (frPct<FR_SQUEEZE) {
    score+=20; sigs.push(`FR ${frPct.toFixed(4)}% negatif kuat = reversal segera`);
  } else if (frPct<FR_NEG) {
    score+=12; sigs.push(`FR ${frPct.toFixed(4)}% negatif = bearish sentiment = contrarian buy`);
  } else if (frPct<FR_COOL) {
    score+=6; sigs.push(`FR ${frPct.toFixed(4)}% di bawah neutral = mild bullish`);
  } else if (frPct<FR_NEUTRAL) {
    score+=2;
  } else if (frPct>FR_HOT) {
    score-=24; sigs.push(`FR +${frPct.toFixed(4)}% OVERHEATED = dump risk`); opp='SHORT_SETUP';
  } else if (frPct>FR_ELEVATED) {
    score-=13; sigs.push(`FR +${frPct.toFixed(4)}% elevated = hati-hati`);
  }

  // Retail positioning (contrarian)
  const rl=retailLong||50;
  if (rl<=32) { score+=22; sigs.push(`${100-rl}% retail SHORT = pump akan paksa cover`); opp=opp||'SQUEEZE_PLAY'; }
  else if (rl<=40) { score+=10; sigs.push(`${100-rl}% retail short = mild contrarian buy`); }
  else if (rl>=68) { score-=18; sigs.push(`${rl}% retail LONG = SM akan jual ke mereka`); opp=opp||'SHORT_SETUP'; }

  return {
    score:cl(score,-60,100), state, stateColor,
    signals:sigs.slice(0,4), opportunity:opp,
    frPct:+frPct.toFixed(5), oiB:+(oi/1e9).toFixed(3),
  };
}

// ─── ATR-BASED SL/TP (per koin, real data) ───────────────────────
function buildLevels(price, dir, atrPct, prob) {
  // Real ATR dari klines, atau fallback per price range
  const hasReal = atrPct > 0;
  const a = hasReal ? atrPct : (price>10000?2.0:price>1000?2.3:price>10?2.8:price>1?3.2:4.0);

  const slP = +(a*1.3).toFixed(2);
  const t1P = +(a*1.9).toFixed(2);
  const t2P = +(a*3.3).toFixed(2);
  const t3P = +(a*5.5).toFixed(2);
  ...
  if (dir==='LONG') return {
    sl:+fmt(price*(1-slP/100)),tp1:+fmt(price*(1+t1P/100)),
    tp2:+fmt(price*(1+t2P/100)),tp3:+fmt(price*(1+t3P/100)),
    slPct:slP,tp1Pct:t1P,tp2Pct:t2P,tp3Pct:t3P,rr,kellySizePct:kelly,realATR:hasReal,
  };
  return {
    sl:+fmt(price*(1+slP/100)),tp1:+fmt(price*(1-t1P/100)),
    tp2:+fmt(price*(1-t2P/100)),tp3:+fmt(price*(1-t3P/100)),
...
// ─── CACHE (dengan OI delta tracking) ────────────────────────────
...
export default async function handler(req, res) {
  ...
  // Cache valid HANYA jika punya real RSI (fix bug cache 0 RSI)
  if (CACHE.d && (t0-CACHE.t)<90000 && (CACHE.d.dataQuality?.realRSI||0)>=10) {
    return res.status(200).json({...CACHE.d, cached:true, elapsed:Date.now()-t0});
  }

  try {
    // ── PHASE 1: PARALLEL MEGA-FETCH ─────────────────────────────
    const [byLinR,bySpR,mexcR,fgR,btcLSR] = await Promise.allSettled([
      sf('https://api.bybit.com/v5/market/tickers?category=linear',5000),
      sf('https://api.bybit.com/v5/market/tickers?category=spot',4000),
      sf('https://api.mexc.com/api/v3/ticker/24hr',4000),
    ...
    // Bybit linear (primary — punya FR + OI)
    try {
      for (const t of A(byLinR.value?.result?.list)) {
        ...
        const v = N(t.turnover24h);
        if (v < MIN_VOL_MAP) continue;
        const prev = N(t.prevPrice24h||p);
        const c24 = prev>0?+((p-prev)/prev*100).toFixed(3):0;
        const hi=N(t.highPrice24h||p*1.01), lo=N(t.lowPrice24h||p*0.99);
    ...
    try {
      for (const t of A(mexcR.value||[])) {
        ...
        const pip=hi>lo?cl((p-lo)/(hi-lo)*100,0,100):50;
        cm[s]={p,fr:FR_NEUTRAL/100,frPct:FR_NEUTRAL,oi:0,oiChg:0,c24,v,h:hi,l:lo,pip,rLong:50,rShort:50,src:'mexc'};
      }
    } catch {}

    // Simpan OI sekarang untuk delta berikutnya
    const currentOI = {};
    for (const [s,raw] of Object.entries(cm)) currentOI[s] = raw.oi||0;
    ...
    try {
      ...
      if(row){btcLongPct=+(N(row.buyRatio)*100).toFixed(2);btcShortPct=+(N(row.sellRatio)*100).toFixed(2);btcLS=btcShortPct>0?+(btcLongPct/btcShortPct).toFixed(3):null;}
    } catch {}

    // ── PHASE 3: PREMIUM KLINES (20 coins, 4500ms) ────────────────
    const premCoins = PREMIUM.filter(s => cm[s]);
    const [btcD1Res,...kRes] = await Promise.allSettled([
      sf('https://api.bybit.com/v5/market/kline?category=linear&symbol=BTCUSDT&interval=D&limit=35',4500),
      ...premCoins.map(s =>
        sf(`https://api.bybit.com/v5/market/kline?category=linear&symbol=${s}USDT&interval=240&limit=60`,4500)
      ),
    ]);

    const kMap = {};
    for (let i=0; i<premCoins.length; i++) {
      const sym = premCoins[i];
      try {
        const r = kRes[i];
        if (r.status!=='fulfilled'||!r.value) continue;
        const d = r.value;
        // retCode 0 = success; retCode undefined = juga ok (beberapa endpoint)
        if (d.retCode!==undefined&&d.retCode!==0) continue;
        ...
        const macdV = macd(cls);
        const atrV = atr(K);
        const atrPct = lp>0?+(atrV/lp*100).toFixed(3):0;
        // OBV (20 candle terakhir)
        let obvUp=0,obvDn=0;
        for (let j=Math.max(1,K.length-20);j<K.length;j++) {
          if(K[j].c>K[j-1].c) obvUp+=K[j].q; else if(K[j].c<K[j-1].c) obvDn+=K[j].q;
        }
        // ATR compression: last 5 vs last 20
        const atr5 = K.length>=7?atr(K.slice(-6),5):atrV;
        const atr20= K.length>=22?atr(K.slice(-21),14):atrV;
        const isCoiling = atr5>0&&atr20>0&&atr5<atr20*0.62;
        // RSI direction
        const rsiPrev = cls.length>=17?rsi14(cls.slice(0,-1)):null;
        const rsiDir  = rsiPrev!==null?(rsiV>rsiPrev+0.5?'up':rsiV<rsiPrev-0.5?'down':'flat'):'flat';
        const e200 = ema(cls,Math.min(200,cls.length));
        const recent = cls.slice(-20);
        kMap[sym]={
          rsi:+rsiV.toFixed(2),rsiDir,macd:macdV,K,cls,
          atr:+atrV.toFixed(10),atrPct,
          obvBull:obvDn>0?obvUp>obvDn*1.1:obvUp>0,
          isCoiling,aboveE200:lp>e200,e200,
    ...
    try {
      ...
      if(d?.retCode===0||d?.result){
        const K=parseKlines(A(d?.result?.list));
    ...
    for (const [sym,raw] of Object.entries(cm)) {
      try {
        ...
        const rsiReal=!!(kd?.ok);
        // RSI: real atau estimasi
        let rsi;
        if (rsiReal) {
          rsi=kd.rsi;
        } else {
          // Estimasi lebih akurat dari multiple signals
          const frEff = (raw.frPct-FR_NEUTRAL)*(-200); // FR di bawah neutral → RSI lebih rendah
        ...
        const lvls  = sigR.direction!=='WAIT'
          ? buildLevels(raw.p,sigR.direction,atrPct,sigR.probability)
          : null;

        // Conviction stars 0-5
        const stars=[
        ...
        coins.push({
          ...
          fr:raw.fr||0, frPct:raw.frPct||FR_NEUTRAL,
          oi:raw.oi||0, oiChg:raw.oiChg||0, pip:raw.pip,
          rs, atrPct, obvBull, isCoiling, macd:macdV,
          retailLong:raw.rLong||50, retailShort:raw.rShort||50,
          ...sigR, conv:convR, whale:whaleR, futures:futR,
          levels:lvls, convStars, bidAsk:raw.bidAsk||50, src:raw.src,
    ...
    if (osPct>0.12) {
      ...
      mcDesc=`${osC} koin RSI<30 (${(osPct*100).toFixed(0)}%). Zona DCA historis. ${whaleC} whale aktif.`;
      mcStrat='DCA Agresif';mcRisk='MODERATE';mcPos='50-75%';
    } else if (obPct>0.18) {
      mcType='MASS OVERBOUGHT';mcColor='red';
      mcDesc=`${obC} koin RSI>70. Distribusi massal. Cash is king.`;
      mcStrat='Cash+Short';mcRisk='HIGH';mcPos='25%';
    } else if (bPct>0.60) {
      mcType='BULLISH';mcColor='green';
      mcDesc=`${Math.round(bPct*100)}% koin bullish. Trend aktif. ${sqC} squeeze.`;
      mcStrat='Aggressive Long';mcRisk='STANDARD';mcPos='100%';
    } else if (bPct<0.28) {
      mcType='BEARISH';mcColor='red';
      mcDesc=`Majority bearish. ${longC}/${total} koin bullish. Cash atau short.`;
      mcStrat='Cash or Short';mcRisk='HIGH';mcPos='25%';
    } else {
      mcType='TRANSITIONAL';mcColor='amber';
      mcDesc=`Mixed. ${Math.round(bPct*100)}% bullish. ${whaleC} whale. ${sqC} squeeze.`;
    ...
    // ── WHALE RADAR ────────────────────────────────────────────────
    const whaleRadar = coins.filter(c=>c.whale.score>=22&&c.oi>=MIN_OI_WHALE)
      .sort((a,b)=>b.whale.score-a.whale.score).slice(0,15)
      .map(c=>({sym:c.sym,price:c.price,c24:c.c24,vol:c.vol,rsi:c.rsi,rsiReal:c.rsiReal,fr:c.frPct,oi:c.oi,whaleScore:c.whale.score,whaleLevel:c.whale.level,whaleSigs:c.whale.signals,whaleTags:c.whale.tags,conv:c.conv.score,signal:c.signal,sector:c.sector}));

    const whaleFingerprint = coins.filter(c=>c.oi>=150e6&&c.frPct<FR_NEUTRAL&&Math.abs(c.c24)<3&&c.vol>2e6)
      .sort((a,b)=>a.frPct-b.frPct).slice(0,8)
      .map(c=>({sym:c.sym,price:c.price,c24:c.c24,rsi:c.rsi,fr:c.frPct,oi:c.oi,vol:c.vol,rating:c.frPct<FR_SQUEEZE?'STRONG':c.frPct<FR_NEG?'GOOD':'WATCH'}));

    const squeezeRadar = coins.filter(c=>c.frPct<FR_SQUEEZE&&c.vol>1e6&&c.rsi<60&&c.oi>=MIN_OI_FUT)
      .sort((a,b)=>a.frPct-b.frPct).slice(0,10)
      .map(c=>({sym:c.sym,price:c.price,c24:c.c24,rsi:c.rsi,fr:c.frPct,oi:c.oi,retailLong:c.retailLong,retailShort:c.retailShort,strength:c.frPct<FR_EXTREME?'EXTREME':c.frPct<FR_NEG?'STRONG':'HIGH'}));

    // ── FUTURES INTEL ──────────────────────────────────────────────
    const futSqueeze = coins.filter(c=>c.frPct<FR_SQUEEZE&&c.vol>1e6&&c.oi>=MIN_OI_FUT)
      .sort((a,b)=>a.frPct-b.frPct).slice(0,12)
      .map(c=>({sym:c.sym,price:c.price,c24:c.c24,frPct:c.frPct,oi:c.oi,rsi:c.rsi,retailLong:c.retailLong,retailShort:c.retailShort,futuresState:c.futures.state,futuresScore:c.futures.score,opportunity:c.futures.opportunity,conv:c.conv.score,signal:c.signal}));

    const futOB = coins.filter(c=>c.frPct>FR_ELEVATED&&c.rsi>65&&c.vol>2e6&&c.oi>=MIN_OI_FUT)
      .sort((a,b)=>b.frPct-a.frPct).slice(0,8)
      .map(c=>({sym:c.sym,price:c.price,c24:c.c24,frPct:c.frPct,rsi:c.rsi,retailLong:c.retailLong}));

    const retailTrap = coins.filter(c=>(c.retailLong||50)>=63&&c.rsi>58&&c.vol>2e6&&c.oi>=MIN_OI_FUT)
      .sort((a,b)=>b.retailLong-a.retailLong).slice(0,8)
      .map(c=>({sym:c.sym,price:c.price,c24:c.c24,retailLong:c.retailLong,rsi:c.rsi}));

    const retailSqueeze = coins.filter(c=>(c.retailLong||50)<=37&&c.rsi<52&&c.vol>2e6&&c.oi>=MIN_OI_FUT)
      .sort((a,b)=>a.retailLong-b.retailLong).slice(0,8)
      .map(c=>({sym:c.sym,price:c.price,c24:c.c24,retailLong:c.retailLong,retailShort:c.retailShort,rsi:c.rsi}));

    // ── GOLDEN OPPORTUNITIES ──────────────────────────────────────
    const stealthVol = coins.filter(c=>c.vol>5e6&&Math.abs(c.c24)<2&&c.oi>=MIN_OI_FUT&&c.rsi<65)
      .sort((a,b)=>b.vol-a.vol).slice(0,8)
      .map(c=>({sym:c.sym,price:c.price,c24:c.c24,vol:c.vol,rsi:c.rsi,oi:c.oi,fr:c.frPct}));

    const hiddenGems = coins.filter(c=>c.rsi<35&&c.frPct<FR_ELEVATED&&c.vol>300000&&c.conv.score>=50&&c.oi>=MIN_OI_WATCH&&!['BTC','ETH','BNB','SOL','XRP','ADA','DOGE'].includes(c.sym))
    ...
    const momentumShift = coins.filter(c=>c.c24>2&&c.rs>4&&c.vol>2e6&&c.rsi<72&&c.sym!=='BTC')
      .sort((a,b)=>b.rs-a.rs).slice(0,8)
      .map(c=>({sym:c.sym,price:c.price,c24:c.c24,rsi:c.rsi,vol:c.vol,rs:c.rs}));

    // ── BTC SNAPSHOT ──────────────────────────────────────────────
    const btcSnap = {
      price:btcP,ch24:btcC24,fg,fgLabel,
      rsi:btcKd.rsi||null, rsiDir:btcKd.rsiDir||'flat',
      atr:btcKd.atr||0, atrPct:btcKd.atrPct||0,
      macd:btcKd.macd||null, aboveEma200:btcKd.aboveE200||false,
      e200:btcKd.e200||0, d1rsi:btcD1rsi?+btcD1rsi.toFixed(1):null,
    ...
    // ── SPOT ACCUMULATION ─────────────────────────────────────────
    const spotAccum = coins.filter(c=>c.rsi<42&&c.direction!=='SHORT'&&c.oi>=5e6&&c.vol>=500000)
      .sort((a,b)=>{
        const sA=(a.rsiReal?10:0)+(a.oi>100e6?5:a.oi>30e6?3:0)+(42-a.rsi);
        const sB=(b.rsiReal?10:0)+(b.oi>100e6?5:b.oi>30e6?3:0)+(42-b.rsi);
        return sB-sA;
      }).slice(0,12).map(c=>{
        const dcaP=Math.max(c.atrPct||0,2.5);
        return{sym:c.sym,price:c.price,rsi:c.rsi,rsiReal:c.rsiReal,signal:c.signal||'OVERSOLD',atrPct:c.atrPct,fr:c.frPct,oi:c.oi,vol:c.vol,dcaZone:`$${+(c.price*(1-dcaP*1.5/100)).toFixed(c.price>1?2:8)} – $${+(c.price*(1-dcaP*0.3/100)).toFixed(c.price>1?2:8)}`,conv:c.conv.score,oversold:c.rsi<25?'EXTREME':c.rsi<30?'DEEP':'MODERATE'};
      });
    ...
    for (const coin of coins) {
      ...
      secMap[s].frSum+=coin.frPct||FR_NEUTRAL;
    ...
    const mkChecks=[
      ...
      {label:'FR market tidak overheated massal',pass:frHotC===0,detail:`${frHotC} koin FR>${FR_ELEVATED}%`,fix:'Hindari entry saat FR mahal'},
    ...
    dos+=Math.min(12,sqC*2.5);
    dos+=Math.min(10,whaleC*2);
    dos+=Math.min(8,realRSIC>=15?8:realRSIC>=8?5:2);
    ...
    const dosAction=dos>=82?'Hari langka! Full sizing PRIME/ELITE.':dos>=67?'Setup bagus. Sizing normal. Conv 70+.':dos>=52?'Selektif. Hanya 3+ konfluens.':'Hindari entry baru.';

    // ── MARKET REGIME ──────────────────────────────────────────────
    const mvrvP=btcKd.cls&&btcKd.e200>0?+(btcP/btcKd.e200).toFixed(3):1.3;
    ...
    // ── BEST TRADE ────────────────────────────────────────────────
    const btTrade=(()=>{
      const cands=coins.filter(c=>c.direction==='LONG'&&c.conv.score>=55&&c.oi>=MIN_OI_FUT)
        .sort((a,b)=>{
      ...
      if(top.frPct<FR_NEG) rsns.push(`FR ${top.frPct}% negatif = contrarian buy`);
      ...
      return{sym:top.sym,price:top.price,signal:top.signal||'—',rsi:top.rsi,fr:top.frPct,conv:top.conv.score,convLabel:top.conv.label,convStars:top.convStars,whale:top.whale,futures:top.futures,mtfConfirmed:!!(top.rsiReal&&top.rsi<38),retailLong:top.retailLong,retailShort:top.retailShort,reasoning:rsns,...top.levels,rr:top.levels?.rr||2.3,probability:top.probability||70,kellySizing:top.levels?.kellySizePct||2,oi:top.oi,vol:top.vol,sector:top.sector};
    })();

    // ── ASSEMBLE ──────────────────────────────────────────────────
    const out={
      ok:true, version:'v20.0', brand:'369 GLOBAL CRYPTO',
      ts:Date.now(), elapsed:Date.now()-t0,
      dataQuality:{coins:total,realRSI:realRSIC,bybitLinear:Object.values(cm).filter(c=>c.src==='bybit').length,mexc:Object.values(cm).filter(c=>c.src==='mexc').length,whaleSignals:whaleC,squeezeSignals:sqC,longCoins:longC,shortCoins:shortC,signalCoins:coins.filter(c=>c.signal&&c.signal.length>0).length},
      fg,fgLabel,
      ...
      checklist:{marketChecks:mkChecks,coinChecks:[{label:'RSI koin <72'},{label:'Conv Score 60+'},{label:'FR <+0.04%'},{label:'RR min 1:2.5'},{label:'Volume >$2M USD'},{label:'OI >$10M'},{label:'Size max 2% equity'},{label:'SL ATR-based real'},{label:'Sesuai skenario Game Plan'},{label:'No entry 30min sebelum news'}],marketPassCount:passC,overallGreenLight:passC>=6,verdict:passC>=6?'KONDISI LAYAK TRADING':`HATI-HATI — ${8-passC} kondisi belum terpenuhi`},
